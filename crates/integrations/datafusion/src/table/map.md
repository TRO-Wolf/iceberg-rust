...
| `uuid_static_tests.rs` | F-UUID-STATIC-2 pins: static provider nested struct/list/map uuid advertisement (`Utf8` at every depth) and canonical rendering, text-vs-byte pushdown file parity for `=`/`IN`/`IS NULL`, option-off byte advertisement and byte rows, catalog propagation with metadata tables resolving unchanged and `deregister_table` keeping the flag |

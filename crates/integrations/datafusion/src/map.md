...
| `schema.rs` | `IcebergSchemaProvider` — `table_names` lists catalog entries only (row R170); `$`-name `table` / `table_exist` still resolve; `with_uuid_as_string(bool)` propagates to every resolved and deregistered `IcebergTableProvider` through one helper (F-UUID-STATIC-2), metadata tables unchanged |

...
| `scan_knobs.rs` | F-27 size-gate split: the session scan knobs (`IcebergScanOptions`, `ScanKnobs`, `scan_knobs_from_context`, `clamp_scan_knob`, `ensure_iceberg_scan_options`), re-exported through `scan.rs` so every existing path is unchanged. `ScanKnobs.uuid_as_string` carries the provider opt-in (F-UUID-STRING-1) into `IcebergTableScan::plan` |

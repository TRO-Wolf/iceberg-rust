...
| `scan_knobs.rs` | F-27 size-gate split: the session scan knobs (`IcebergScanOptions`, `ScanKnobs`, `scan_knobs_from_context`, `clamp_scan_knob`, `ensure_iceberg_scan_options`), re-exported through `scan.rs` so every existing path is unchanged. `ScanKnobs.uuid_as_string` carries the provider opt-in (F-UUID-STRING-1) into `IcebergTableScan::plan`. `advertised_schema` (F-UUID-STATIC-2) is the one byte-or-text schema selector both providers call |
| `scan.rs` text rendering | F-UUID-STRING-1: with the knob on, `plan` advertises the uuid-as-text schema and `execute` renders uuid bytes to canonical text inside the scan (`conform_render_strip`), so pushdown still prunes on bytes |
| `metadata_scan.rs` | `IcebergMetadataScan` — projects inspect batches |
| `project.rs` | partition-value projection |
| `commit.rs` / `write.rs` | INSERT commit; the write exec declares and preserves hash distribution on evaluated partition values, plus partition ordering for clustered writers. `write.rs` also declares the default-sort-order input ordering (so the optimizer keeps the `SortExec`) and stamps it on every data file (row R171). `IcebergWriteExec` emits each file with its `write_partition_index` (the write task's output partition); `IcebergCommitExec` commits the collected list in ascending partition order — the `ascending_partition_order` rule `FanoutWriter::close` uses — tie-broken by that index, so v3 `first_row_id` is deterministic (F-ROWID-ORDER-1). INSERT parquet `WriterProperties` take compression from `parquet_compression_from_properties` (default zstd; table properties `write.parquet.compression-codec` / `write.parquet.compression-level`) and disable dictionary encoding unless table property `parquet.enable.dictionary` reads true (row R174) |
| `commit_order.rs` | session-config `DataFileCommitOrder` hook reordering the commit's data files after the ascending-partition sort; absent hook keeps today's order, non-permutation results fail the commit |
| `commit_order_tests.rs` | hook pins wired from `commit_order.rs`: no-hook ascending order, reverse hook, `ConfigOptions` visibility, unpartitioned single file, drop/duplicate refusal without a snapshot |
| `delete.rs` / `update.rs` | DELETE / UPDATE (F-26: both MoR paths scan once for batches plus the per-path partition map via `mor_scan` and hand it to the V3 close as `known_partitions`; both return the close alongside the row count so tests pin the threading; r2: the map is DV-only and retained to touched paths). F-OUTPUT-SPEC-ID-1: both execs carry the provider-resolved `PartitionSpecRef` and the CoW/MoR-UPDATE data writers build under it |
| `promotion.rs` | F-PROMOTE-READ-1: `widened_batch` rebuilds a scanned batch under the current table schema and widens a column written before a legal promotion (`Int32 → Int64`, `Float32 → Float64`, `Decimal128(p,s) → Decimal128(p',s)`); every other mismatch still fails in `RecordBatch::try_new`. Called from `delete.rs` (`table_column_batch`, the merge-on-read delete predicate batch) |
| `mor_scan.rs` | F-26 r2 size-gate split: the MoR scan seam (`mor_scan_stream`) plus the DV-only partition-map shaping (`dv_partitions_for`), called from `delete.rs`. F-EVO-SCAN-1: the scan projects the current schema over the pinned snapshot's files, so DML survives `ADD`/`RENAME COLUMN` with no write since |
| `delete_position_deletes.rs` | F-26 size-gate split: the V2 position-delete writers (`write_position_deletes`, grouping, per-partition write) in the resolved delete format (`write.delete.format.default`, else the table data format; WO-PR5a), called from `delete.rs` |
| `delete_decode.rs` | F-OUTPUT-SPEC-ID-1 size-gate split: the reserved `_file` / `_pos` column decoders (`decode_file_path`, `decode_file_paths_batch`, `decode_position`), called from `delete.rs` |
...
| `repartition.rs` / `sort.rs` | writer helpers; every partitioned spec hashes the projected `_partition` struct, while unpartitioned input stays round-robin. `sort.rs` additionally resolves the default sort order into a per-partition `SortExec` plus the `sort_order_id` stamp (`sort_for_write` / `write_sort_plan`; row R171). Float/double keys sort through a NaN-canonicalising wrapper; nested struct-child keys resolve by field id (`NestedFieldExpr` with null-parent propagation); unresolvable orders keep the unsorted plan with a 0 stamp; fanout-off without a `_partition` column keeps the input as-is |
| `expr_to_predicate.rs` | filter pushdown (F-ICE-NAN-PUSHDOWN-1: NaN equality and IN rewrite to `is_nan`, never a NaN literal; ranges and NOT IN with NaN stay unpushed; F-LIST-NULL-ACCESSOR-1: `term_binds_soundly` mirrors `Reference::bind` — field plus accessor — so null tests and every other operator on list/map/struct or accessorless leaf names drop to residual evaluation instead of raising `Accessor for Field … not found`; F-UUID-STRING-1: `FixedSizeBinary(16)` literals convert to uuid datums, which is what the provider's uuid-text filter rewrite emits; with the option off this is a pure pruning gain — a byte literal on a uuid column now plans 1 of 2 files where it planned 2, rows unchanged, pinned by `uuid_option_off_byte_literal_prunes_with_unchanged_rows`) |
| `expr_to_predicate_nan_tests.rs` | conversion unit pins for the NaN arms, wired via `#[path]` from `expr_to_predicate.rs` |
| `write_tests.rs` | WO-R20 size-gate split: the whole `write.rs` `mod tests` (INSERT exec, metrics-default, ORC/Avro/Puffin/bogus format pins, parquet dictionary policy), wired via `#[path]` from `write.rs` |

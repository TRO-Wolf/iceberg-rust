...
| `promoted_type_dml.rs` | F-PROMOTE-READ-1: `UPDATE` and `DELETE` (copy-on-write and merge-on-read) on a table whose `id` was promoted `int → long` with no write since — the scan reads `Int32`, the exec rebuilds batches under the current schema; each pin asserts the updated or surviving rows |
| `evo_schema_dml.rs` | F-EVO-SCAN-1: SQL `UPDATE … WHERE id = 1` and `DELETE … WHERE id = 1` (copy-on-write and merge-on-read, two data files) after `ADD COLUMN`, `RENAME COLUMN`, and a two-name swap with no write since — the DML scans project the current schema over the pinned snapshot's files; each pin asserts the post-statement rows. F-LIST-INSERT-1 r2: `INSERT … VALUES` into `list<int>` / `list<struct>` / `map<string,list<int>>` columns (multi-row, NULL row, null element) — the provider advertises a metadata-free planning schema while the write path keeps the stamped one; pins assert the read-back rows and the parquet footer leaf ids |
| `unknown_column_dml.rs` | F-UNKNOWN-WRITE-1: a copy-on-write `DELETE` on a v3 table with an `unknown` column rewrites a file whose Parquet footer holds only the written columns and whose `column_sizes`, counts and bounds maps omit the unknown field id |
...
| `sorted_insert.rs` | F-SORTED-INSERT-1: INSERT into a table with a default sort order writes per-file sorted data stamped with the order id (asc, desc, two-key nulls, partitioned, multi-stream, bucket transform, float/double NaN ASC/DESC, unknown-transform zero-stamp; row R171). Unsorted tables add no `SortExec` and stamp 0 |
| `sorted_insert_types.rs` | F-SORTED-INSERT-1 round 2: string, decimal, timestamptz, boolean, binary identity keys; truncate string/decimal; day/hour; NULL through bucket (row R171) |
| `sorted_insert_writer.rs` | F-SORTED-INSERT-1 round 2: nested single and two-key sort, rolling split, Overwrite, fanout disabled (row R171) |
| `sorted_insert_shared/` | Shared fixture, file/stamp readers shared by the three sorted-insert binaries |
...
| `count_star_fold.rs` | F-27b (OFFLINE): `IcebergTableScan::partition_statistics` reports exact whole-table row counts (the planned data files' path-deduped manifest record counts, refused on deletes or real residuals) so DataFusion folds `count(*)` without a scan. Plain table: Exact(3), answer 3, physical plan has no `IcebergTableScan`; V3 MoR table with a DV: unknown, answer 2, plan still scans; residual filter / limit / per-partition query: unknown; empty table: Exact(0); COW DELETE (no delete files): Exact(2) |
...
| `target_file_size.rs` | F-TARGET-FILE-SIZE-1: oracle-shape INSERT (200k rows over 4 streams) writes 20 files within ±25% of the Spark file count and max size at target 262144; 1 MiB and default targets keep one file per stream; created tables carry the stamped zstd codec (row R174, row R175) |
| `nan_pushdown.rs` | F-ICE-NAN-PUSHDOWN-1: two-file MemoryCatalog table with NaN, finite and NULL double and float rows; SQL equality, IN, range, NotEq and null-safe equality over CAST NaN literals return the Spark 4.1.2 id sets |
...
| `stage_only.rs` | F-STAGE-ONLY-2: `IcebergTableProvider::with_stage_only(true)` stages SQL INSERT and INSERT OVERWRITE (new snapshot, `main` unmoved, staged op/parent pinned, main scan unchanged); staged commit on a named branch leaves the branch unmoved; near misses (plain append/overwrite advance `main`, branch-only still targets the branch). F-STAGE-ONLY-2b: `with_snapshot_properties` merges caller entries into the committed summary on both arms (operation id survives, a caller operation id loses); staged + properties composes and resolves via the `wap.id` lookup |
| `mor_delete_format.rs` | WO-PR5a (D-4 site 3): V2 merge-on-read DELETE resolves the delete format (`write.delete.format.default`, else the table data format). ORC and Avro tables take same-format deletes (files table + suffix + magic bytes); the property overrides independently (ORC data, parquet deletes, surviving rows); v3 stays Puffin for parquet/orc/avro data; garbage and puffin delete formats refuse typed; a two-partition delete returns every delete file; parquet deletes keep full exact `file_path` bounds, the `delete-type=position` stamp, and file_path-dict-only encoding |
...
| Pin `with_stage_only` staged INSERT / OVERWRITE / `with_snapshot_properties` summary stamp | `stage_only.rs` |
...
| Pin target-file-size Spark fidelity | `target_file_size.rs` |

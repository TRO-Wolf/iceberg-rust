...
use crate::catalog::S3TablesCatalog;
use crate::commit_transport::{
    DiscardingS3TablesCommitTransport, S3TablesCommitScript, S3TablesCommitTransport,
    ScriptedS3TablesCommitTransport,
};
...
async fn dummy_s3tables_client() -> aws_sdk_s3tables::Client {
...
async fn catalog_with(
    scripts: impl IntoIterator<Item = S3TablesCommitScript>,
    format: FormatVersion,
) -> (
    S3TablesCatalog,
    Table,
    Arc<ScriptedS3TablesCommitTransport>,
    FileIO,
    TableIdent,
) {
    ...
    let scripted = ScriptedS3TablesCommitTransport::new(scripts);
    let client = dummy_s3tables_client().await;
    let catalog = S3TablesCatalog::for_commit_outcome_tests(
        file_io.clone(),
        Arc::clone(&scripted) as Arc<dyn S3TablesCommitTransport>,
        table.clone(),
        client,
    );
...
#[tokio::test]
async fn never_sent_is_terminal_for_every_commit_class_on_the_shared_s3tables_path() {
    for class in COMMIT_CLASSES {
        let scripts = if class.needs_seed_append() {
            vec![
                S3TablesCommitScript::Success,
                S3TablesCommitScript::StopBeforeSend,
            ]
        } else {
            vec![S3TablesCommitScript::StopBeforeSend]
        };
...
async fn maybe_sent_is_unknown_without_a_second_commit_for_every_commit_class() {
    for class in COMMIT_CLASSES {
        let scripts = if class.needs_seed_append() {
            vec![
                S3TablesCommitScript::Success,
                S3TablesCommitScript::MaybeSentLost,
            ]
        } else {
            vec![S3TablesCommitScript::MaybeSentLost]
        };
...
async fn accepted_then_lost_append_reconciles_without_a_duplicate_commit() {
    let (catalog, table, scripted, _, _) =
        catalog_with([S3TablesCommitScript::AcceptThenLose], FormatVersion::V2).await;
...
async fn accepted_then_lost_without_the_snapshot_stays_unknown() {
    let (catalog, table, scripted, _, _) =
        catalog_with([S3TablesCommitScript::MaybeSentLost], FormatVersion::V2).await;
    let before = table.metadata().snapshots().count();
...
async fn cas_conflict_rebases_when_the_append_validation_contract_permits() {
    ...
    let committed = commit_class(&catalog, table, CommitClass::SnapshotAppend)
        .await
        .expect("conflict then success is a permitted rebase even with S3 Tables service-side maintenance");
...
async fn permanent_authorization_denial_is_terminal_and_does_not_clean_staged_files() {
    let (catalog, table, _, file_io, _) =
        catalog_with([S3TablesCommitScript::Forbidden], FormatVersion::V2).await;
    let before_files = count_metadata_files(&file_io, &table).await;
    let error = commit_class(&catalog, table.clone(), CommitClass::SnapshotAppend)
        .await
...
#[tokio::test]
async fn credentialed_s3tables_commit_class_smokes_and_one_accepted_then_lost_append() {
    if !credentialed_requested() {
        assert_ne!(std::env::var(CREDENTIALED_ENV).ok().as_deref(), Some("1"));
        return;
    }
    let arn = std::env::var("ICEBERG_PR5A_S3TABLES_BUCKET_ARN").unwrap_or_default();
    assert!(
        !arn.is_empty(),
        "ICEBERG_PR5A_CREDENTIALED=1 requires ICEBERG_PR5A_S3TABLES_BUCKET_ARN"
    );
    ...
    let catalog = S3TablesCatalogBuilder::default()
        .load("pr5a-s3tables", props)
        .await
        .expect("load s3tables catalog");
    ...
    let discarding = Arc::new(DiscardingS3TablesCommitTransport::new(
        catalog.live_commit_transport(),
    ));
    let discarded =
        catalog.with_commit_transport(Arc::clone(&discarding) as Arc<dyn S3TablesCommitTransport>);
...
async fn discarding_transport_marks_accepted_response_lost() {
    let (catalog, _, _, _, _) =
        catalog_with([S3TablesCommitScript::Success], FormatVersion::V2).await;
    let discarding = Arc::new(DiscardingS3TablesCommitTransport::new(
        catalog.live_commit_transport(),
    ));
    let catalog =
    ...
    let discarding = DiscardingS3TablesCommitTransport::new(ScriptedS3TablesCommitTransport::new([
        S3TablesCommitScript::Success,
    ])
        as Arc<dyn S3TablesCommitTransport>);
    let send = discarding
        .send_update_metadata_location(crate::commit_transport::S3TablesUpdateCall {
            table_bucket_arn: "arn:aws:s3tables:us-east-1:123456789012:bucket/pr5a".to_string(),
            namespace: "pr5a".to_string(),
            name: "t".to_string(),
            version_token: "v0".to_string(),
            metadata_location: "memory://pr5a/t/metadata.json".to_string(),
        })
        .await;
    assert!(matches!(
        send,
        crate::commit_transport::S3TablesCommitSend::AcceptedResponseLost
    ));

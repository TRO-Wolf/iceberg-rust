...
| `src/catalog/test_support.rs` | `#[cfg(test)]` catalog seams extracted from `catalog.rs`: `catalog_commit_attempts`, `with_commit_transport` / `live_commit_transport`, `for_commit_outcome_tests_at_version` (injectable pointer version id, `Some("v0")` via `catalog_with` — a `None` GetTable version means the send carries `version_id: None`, no Glue OCC), `for_commit_fault_tests_at_version` (property-path transport build over an injectable inner, returns the installed fault wrapper). |
| `src/commit_transport.rs` | Narrow seam around the completed Glue `UpdateTable` SDK call. Live / discarding / scripted transports. Classifier feed + service-error mapping. The scripted transport records the last `GlueUpdateTableCall` (`last_call()` → version id + `TableInput` parameters) so pins can see the CAS token and `metadata_location`/`previous_metadata_location` wires. `glue.fault.drop-update-table-response` installs `DiscardingGlueCommitTransport` (next-`n` drops) around the live transport under cargo feature `commit-fault-injection` and refuses construction without it; `map_glue_commit_send_identified` attaches the `engine.operation-id` stamp to `CommitStateUnknown` context. |
...
| `src/commit_fault_tests.rs` | Offline proofs for the property-installed fault seam: dropped append reconciles with one commit attempt, metadata-only stays `CommitStateUnknown` naming the operation id, `n=2` drops then passes, no-feature refusal, unset property unchanged. |

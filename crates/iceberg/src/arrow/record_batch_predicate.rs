...
mod tests {
    ...
    /// Iceberg schema for the container-null pins: the run-25d oracle's column shapes — `st`
    /// STRUCT<a,b> (10/11/12), `xs` ARRAY<INT> (13/14), `mp` MAP<STRING,INT> (15/16/17), `deep`
    /// STRUCT<inner STRUCT<x, ys ARRAY<INT>>> (18/19/20/21/22).
    ...
    /// The oracle's four rows as a materialized batch: row 1 fully populated; row 2 NULL in `st`,
    /// `xs`, `mp`, `deep`; row 3 present-but-empty containers (`xs`/`mp` empty, `st.a` NULL,
    /// `deep.inner` present with `x`/`ys` NULL); row 4 populated with `deep.inner` NULL.
    fn container_truth_batch() -> RecordBatch {
        ...
        let st_field =
            Field::new("st", DataType::Struct(Fields::from([a_field.clone(), b_field.clone()])), true)
        ...
        let xs_field = Field::new("xs", DataType::List(xs_element.clone()), true)
        ...
        let value_field =
            Arc::new(Field::new("value", DataType::Int32, true).with_metadata(fid(17)));
        let entries_field = Arc::new(Field::new(
            "entries",
            DataType::Struct(Fields::from([key_field.clone(), value_field.clone()])),
            false,
        ));
        let mp_field = Field::new("mp", DataType::Map(entries_field.clone(), false), true)
        ...
        let ys_field = Arc::new(
            Field::new("ys", DataType::List(ys_element.clone()), true).with_metadata(fid(21)),
        );
        ...
        let deep_field =
            Field::new("deep", DataType::Struct(Fields::from([inner_field.clone()])), true)
    ...
    /// The run-25d oracle cells, evaluated against the materialized batch (the post-decode
    /// residual path). An EMPTY `xs`/`mp` is NOT NULL (row 3) and a struct whose children are all
    /// NULL is NOT NULL (row 3 `deep.inner` under `deep`... row 3 keeps `inner` present).
    #[test]
    fn container_null_predicates_match_spark_oracle_on_materialized_batch() {
        ...
        let cases: Vec<(&str, crate::expr::Predicate, Vec<bool>)> = vec![
            ...
            (
                "xs IS NULL",
                Reference::new("xs").is_null(),
                vec![false, true, false, false],
            ),
            (
                "mp IS NULL",
                Reference::new("mp").is_null(),
                vec![false, true, false, false],
            ),
            (
                "st.a IS NULL",
                Reference::new("st.a").is_null(),

...
| Understand merge-on-read reads | `task.rs` delete attachments → applied in `../arrow/` (delete_file_index / delete_vector at crate root). **FK2.3:** `../delete_file_index.rs` keys partition maps by `(spec_id, partition)` and seq-sorts lists once so lookup can `partition_point` the applicable tail (wrong key = delete resurrection; multi-spec identical-result pins) |

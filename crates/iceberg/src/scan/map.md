...
| A split sub-task reads the WRONG BYTES of a file (rows from outside its window) | Check `task.start` on the PARENT that was split. `split`'s offsets-aware and fixed-size branches both treat the byte space as ABSOLUTE FROM ZERO, so re-splitting an already-ranged task would relocate its window to offset 0 — re-reading the prefix it never owned and dropping its tail. `task.rs::split` therefore returns `[self]` for `start != 0`; Java forecloses the shape structurally (its `SplitScanTask` is not `SplittableScanTask`), this crate uses ONE public type for both. Pins: `scan::task::tests::split_of_an_already_ranged_task_is_a_passthrough_not_a_relocation` + `arrow::reader::tests::test_split_of_a_ranged_task_reads_the_parents_rows_not_the_whole_file` |

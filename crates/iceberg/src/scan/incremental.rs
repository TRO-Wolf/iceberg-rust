...
//!
//! An [`IncrementalAppendScan`] returns the data files APPENDED between two
//! snapshots — the range `(from_snapshot_id exclusive, to_snapshot_id inclusive]` —
//! considering ONLY `APPEND`-operation snapshots. Overwrites and deletes that landed
//! in the range are excluded, and no delete files are applied. This is the
//! incremental / change-data read primitive.
//!
//! This is a SEPARATE planner from [`TableScan`](super::TableScan): it does not touch
//! the single-snapshot scan's [`plan_files`](super::TableScan::plan_files) and reuses
//! the existing [`PlanContext`] / [`ManifestEntryContext`] machinery (partition-filter
//! pruning, the residual evaluator, and `into_file_scan_task`) to build each
//! [`FileScanTask`].
//!
//! Mirrors Java `BaseIncrementalAppendScan`
...
use crate::scan::{
    BoundPredicates, ExpressionEvaluatorCache, FileScanTask, FileScanTaskStream,

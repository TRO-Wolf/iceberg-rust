...
fn write_posdel_parquet(
    ...
    let mut fields = vec![
        ...
        Field::new("pos", DataType::Int64, false).with_metadata(field_id_meta(DELETE_FILE_POS_ID)),
    ];
    let mut columns: Vec<ArrayRef> = vec![
        Arc::new(StringArray::from_iter_values(
            rows.iter().map(|row| row.0.clone()),
        )),
...
async fn write_manifest_list(fixture: &TableTestFixture, manifests: Vec<ManifestFile>, v3: bool) {
    let current_snapshot = fixture
        .table
        .metadata()
        .current_snapshot()
...
async fn scan_partitioned_v2_rows_match_oracle() {
    ...
    for index in 0..2 {
        assert!(
            rows.is_null(index),
            "row must be NULL when the file stores none"
        );
    }
}
...
async fn scan_partitioned_v3_dv_rows_match_oracle() {
    ...
    let (puffin_size, coordinates) = write_dv_puffin(&fixture, &puffin_path, &[
        (&data_a, &[1_u64]),
        (&data_b, &[7_u64]),
    ])
    .await;
...
async fn scan_evolved_two_specs_null_fills_unified_partition() {
    ...
    .await;
    write_manifest_list(&fixture, vec![manifest_spec_zero, manifest_spec_one], false).await;
...
async fn scan_row_column_is_read_when_the_file_carries_it() {
    ...
    let size_rowed = write_posdel_parquet(&del_rowed, &[(data_b.clone(), 2)], Some(&row_batch));

...
| `partitions.rs` | `PartitionsTable` | the aggregating table: live entries grouped by partition struct; `last_updated_*` via strict-`>` tie-break; shares `data_file.rs`'s field-id-matched `append_partition` for the `partition` column |
...
| See known divergences from Java | GAP_MATRIX inspection rows: unpartitioned `partition` drop now matches Java on files/entries/partitions (row R142, 2026-08-14); timestamptz / timestamptz_ns now project from `.files` / `.partitions` (row R162; interop pending); the Java `Partitioning` unifier now lives in `spec/partitioning.rs` (PT increment A, G1–G4 fixed) but inspect tables still project `default_partition_type` (adoption is increments B/C/D — a partition field only an OLDER spec carries has no column and is dropped; the tuple READ is field-id matched as of PT-0); `readable_metrics` sub-field order is ascending-field-id (Java: HashMap order); `position_deletes` metadata table is unported (row R142) |
...
| A partition value appears under the WRONG partition column, or a partition column is unexpectedly null, on an evolved table | `append_partition` matches the projected field's **id** against the file's own spec's field ids (`DataFile::partition_spec_id` → `partition_field_ids_by_spec`). A positional walk silently swaps same-typed fields (PT-0). An all-null column means the projected (default-spec) field id is absent from that file's spec — expected until inspect adopts `spec/partitioning.rs` (increments B/C/D) |

...
| See known divergences from Java | GAP_MATRIX inspection rows: unpartitioned `partition` drop now matches Java on files/entries/partitions (row R142, 2026-08-14); timestamptz / timestamptz_ns now project from `.files` / `.partitions` (row R162; interop pending); cross-spec `Partitioning.partitionType` unification deferred (the PROJECTED type is still `default_partition_type`, so a partition field only an OLDER spec carries has no column and is dropped — the tuple READ is field-id matched as of PT-0, so nothing is misattributed); `readable_metrics` sub-field order is ascending-field-id (Java: HashMap order); `position_deletes` metadata table is unported (row R142) |

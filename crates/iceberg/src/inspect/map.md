...
| See known divergences from Java | GAP_MATRIX inspection rows: unpartitioned `partition` drop now matches Java on files/entries/partitions (row R142, 2026-08-14); timestamptz / timestamptz_ns now project from `.files` / `.partitions` (row R162; interop pending); `partitions` (B) and `position_deletes` schema (D) adopt `spec/partitioning.rs` — files/entries still project `default_partition_type` (C); `readable_metrics` sub-field order is ascending-field-id (Java: HashMap order); `position_deletes` scan unported (row R142) |
...
| A partition value appears under the WRONG partition column, or a partition column is unexpectedly null, on an evolved table | `partitions` now coerces into the unified type before rollup (increment B). `files`/`entries` still project `default_partition_type` — an all-null column there means the projected (default-spec) field id is absent from that file's spec until increment C. `append_partition` matches by field id (PT-0) |

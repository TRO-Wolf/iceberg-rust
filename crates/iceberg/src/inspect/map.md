...
| `position_deletes.rs` | `PositionDeletesTable.calculateSchema` | **schema only** (FB-2): fixed `MetadataColumns` ids, partition child-id reassignment, empty-partition `partition` drop; partition type is the unified type (increment D); `scan()` refused loud (`FeatureUnsupported`) — increment D does **not** un-refuse the scan |

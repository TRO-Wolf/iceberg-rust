...
| See known divergences from Java | GAP_MATRIX inspection rows: unpartitioned `partition` drop now matches Java on files/entries/partitions (row R142, 2026-08-14); timestamptz / timestamptz_ns now project from `.files` / `.partitions` (row R162; interop pending); `partitions` adopts `spec/partitioning.rs` (PT increment B) — files/entries/`position_deletes` still project `default_partition_type` (C/D); `readable_metrics` sub-field order is ascending-field-id (Java: HashMap order); `position_deletes` metadata table is unported (row R142) |

...
complete `MetadataTableType` vocabulary — `position_deletes` now scans as well as schemas
(F-POSDEL-SCAN-1, row R142). Entry
...
| `files.rs` | `BaseFilesTable` | `files` / `data_files` / `delete_files` — differ only by manifest-content filter (`FilesTableKind`); also hosts the `all_*` variants via `MetadataScope`. Projects the unified partition type (increment C); `try_*` stores it so `schema()` stays infallible |
| `entries.rs` | `ManifestEntriesTable` | every entry **including `Deleted` tombstones** (status==2); nests the shared data_file struct. Projects the unified partition type (increment C) |
| `data_file.rs` | `DataFile.getType` / `PartitionUtil.coercePartition` | shared data_file projection (`files` flattens it, `entries` nests it). Partition tuple is read BY FIELD ID against the file's OWN spec (`partition_field_ids_by_spec` + `append_partition`, PT-0 2026-08-15) — Java `StructProjection.createAllowMissing` semantics, absent projected fields null-fill. Projected type is the unified type (increment C; `append_partition` itself unchanged). Partition-struct append: `Timestamptz` / `TimestamptzNs` project (row R162); `Uuid` / `Fixed` stay `FeatureUnsupported` |
| `count_map.rs` | `DataFile.getType` metrics maps | `append_count_map` / `append_count_map_or_null_for_pos_delete` (metrics-count maps; a position-delete row with an empty map renders NULL, split out of `data_file.rs` for the file-size ceiling) |
| `pos_delete_counts_tests.rs` | — | **test-only** (`#[cfg(test)]`): position-delete rows render NULL field counts, data rows render empty maps as empty maps |
| `partition_values.rs` | `PartitionUtil.coercePartition` (value half) + `Comparators.forType` | shared partition-tuple projection (`append_partition`, split out of `data_file.rs` for the file-size ceiling): field-id match against the file's own spec, per-slot legal-promotion widening (`int → long`, `float → double`) before the Arrow extract, then the `partitions` row-ordering comparators (moved from `partitions.rs`). A `binary` partition value appends through `LargeBinaryBuilder`, matching `type_to_arrow_type` (F-META-BINARY-PART-1) |
...
| `partitions.rs` | `PartitionsTable` | the aggregating table: live entries grouped by **coerced** partition struct (Java `PartitionUtil.coercePartition` into `Partitioning.partitionType`, increment B); `last_updated_*` via strict-`>` tie-break; `try_new` stores the unified type so `schema()` stays infallible; `append_partition` projects the already-coerced tuple by field id (PT-0) |
| `partition_summary.rs` | — | shared partition-summary builder (used by `manifests` + `all_manifests`) |
| `readable_metrics.rs` | `MetricsUtil.readableMetricsStruct` / `readableMetricsSchema` | virtual per-leaf-column typed-metrics struct; bounds decoded via `Datum::try_from_bytes`; field ids seeded at host table's `highestFieldId()`; all six metric leaves carry their Java `READABLE_METRIC_COLS` doc strings (F-1, 2026-08-15 — Java's 4-arg `optional(id, name, type, doc)`) |
| `java_schema_shape.rs` | `MetadataTableUtils` / `BaseFilesTable.schema` / `PartitionsTable.schema` / `BaseEntriesTable.schema` / `MetricsUtil.readableMetricsSchema` | **test-only** battery: Java-cited field id/name/required pins. Increment 1 — files-family (all six analogues) + `partitions`. Increment 2 — `entries`/`all_entries` shape (`ManifestEntry.wrapFileSchema` interleave, nested `selectNot(102)`) and `readable_metrics` STRUCTURE (per-leaf-column struct, six `READABLE_METRIC_COLS`, by-name emit order, host-`highestFieldId` id seed), plus the six leaf DOC strings (F-1: found here, fixed in `readable_metrics.rs`). OUT: `position_deletes` rows, cross-spec unification. Interior field-id ORDER is a documented divergence, pinned as such (Java's HashMap order is not portable) |
...
| See known divergences from Java | GAP_MATRIX inspection rows: unpartitioned `partition` drop now matches Java on files/entries/partitions (row R142, 2026-08-14); timestamptz / timestamptz_ns now project from `.files` / `.partitions` (row R162; interop pending); `partitions` (B), `position_deletes` schema (D), and files/entries (C) adopt `spec/partitioning.rs`; `readable_metrics` sub-field order is ascending-field-id (Java: HashMap order); `position_deletes` scan has no filter-parameter plumbing (F-POSDEL-SCAN-1 — residual pushdown lands with the filter unit, row R142) |
...
| `partition child builder at index N has an unexpected type` on a metadata-table scan | The `append_partition_field` builder for that primitive does not match `type_to_arrow_type` (e.g. `binary` is Arrow `LargeBinary`, not `Binary`); keep the append match aligned with `readable_metrics.rs` |
...
| A partition value appears under the WRONG partition column, or a partition column is unexpectedly null, on an evolved table | `partitions` coerces into the unified type before rollup (B). `files`/`entries` project the unified type (C); `append_partition` matches by field id (PT-0) and null-fills projected fields the file's spec lacks |
| `DataInvalid` `… written under partition spec id N, which is not in the table metadata` | The manifest's `DataFile::partition_spec_id` has no entry in `TableMetadata::partition_specs`; Java NPEs in the same place (`table.specs().get(specId)`) |

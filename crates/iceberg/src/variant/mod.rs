...
//! The Iceberg V3 variant binary format — READ side (decoding).
//!
//! A Rust port of the deserialization surface of Java's `org.apache.iceberg.variants` package,
//! pinned to **iceberg-api 1.10.0** (bytecode-verified; the MAIN-branch sources matched 1.10.0
//! for the entire read surface at port time). A variant is a (metadata, value) pair: the
...
//! - **Value `sizeInBytes()`/`writeTo` are deferred to the write side (B2).** Java's
//!   `SerializedValue` reports its backing buffer's `remaining()`; the eager representation
//!   has no backing buffer. [`VariantMetadata::size_in_bytes`] IS provided (it delimits the
//!   metadata region inside a concatenated variant buffer, which [`Variant::from_bytes`]
//!   needs).
...
//! plane.
//!
//! Out of scope for the read side (deferred, tracked in `docs/parity/GAP_MATRIX.md`): the
//! write side (`Variants` factory, `PrimitiveWrapper`, `ValueArray`, `ShreddedObject`),
//! shredding, the `variant` schema-type integration, and Java↔Rust interop fixtures beyond

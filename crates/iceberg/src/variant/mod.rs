...
//! shredded-parquet FILE I/O and file-level Java↔Rust interop, beyond the 1.10.0-generated byte
//! pins in `tests.rs`. That deferral is a FEATURE-GATE boundary, not a version floor: `parquet`
//! carries the variant logical type and its arrow-side extension path behind the opt-in
//! `variant_experimental` feature (which pulls `parquet-variant{,-json,-compute}`), present and
//! equivalent across the 57 and 58 lines and not enabled by this workspace; `arrow-schema` ships
//! no canonical variant extension type of its own, so the Arrow conversion errors loudly
//! (`arrow/schema.rs`). Enabling the feature would additionally require fork-side work — an
//! Iceberg→`ShreddedSchemaBuilder` bridge and file-level interop. See GAP_MATRIX row R88.

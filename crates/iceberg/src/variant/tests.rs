...
//! Variant decode and encode tests: per-primitive vectors, object and array semantics, the
//! malformed-input security boundary, and the Java-1.10.0-pinned fixture bytes.
//!
//! Fixture provenance: Java generators produced every `JAVA_*` and write-side constant on
//! 2026-06-11 against the pinned iceberg 1.10.0 jars. Each value comes from a
//! `Variants.<factory>(...)` call serialized by `writeTo`, and Java re-read it before it was
//! pinned here. Large fixtures pin `java.util.zip.CRC32` + length + the first 64 bytes.
//!
//! To regenerate a fixture, compile the generator against these pinned artifacts from `~/.m2`:
//!
//! | Generator | Classpath (`CP`) |
//! |---|---|
//! | `VariantFixtureGen` | iceberg-api, iceberg-core, iceberg-bundled-guava 1.10.0, slf4j-api 2.0.17 |
//! | `VariantWriteFixtureGen`, `VariantShredFixtureGen` | `CP` plus avro 1.12.0 and caffeine 3.0.5 |
//!
//! Run `javac -encoding UTF-8 -cp "$CP"`, then `java -Dfile.encoding=UTF-8 -cp "$CP:."`.
...
// ===== objects ===============================================================================
// Layout per Java `SerializedObject`: header bits for offset size (2..4), field-id size (4..6),
// and is-large (6), then the field count, the field-id list, the offset list with one extra
// entry for the data length, then the concatenated field values.
...
// Java 1.10.0 produced or accepted the bytes below. Each test quotes the behavior it pins.

/// Risk pinned (THE UTF-16 comparator trap): Java sorts object fields by `String.compareTo`, so
/// the supplementary 😀 (U+1F600) sorts BEFORE the BMP U+FFFF. Java wrote these bytes in that
/// order and finds both fields. A byte-order comparator probes left at 😀 and MISSES U+FFFF.
#[test]
...
/// Risk pinned: the object is-large bit is bit 6, not bit 4, which belongs to the field-id-size
/// field. A non-large object with 2-byte field ids (header 0x12) exposes a transposed bit, which
/// misreads it as large and consumes a 4-byte count. Java decodes these bytes as {a: int8 34}.
#[test]
...
/// Risk pinned (accepted-set parity): an offset SPAN larger than the value inside it is legal,
/// because Java's lazy reads never touch the slack. The eager parse must ignore trailing bytes
/// inside a field span, exactly as it does at top level.
#[test]
...
/// Risk pinned (documented divergence): Java 1.10.0 accepts three spec-violating shapes that
/// this port rejects at the door. A zero-count object or array can drop its mandatory final
/// offset entry (`[0x02, 0x00]` / `[0x03, 0x00]`). An empty-dictionary metadata can declare a
/// string-data end past the buffer (`[0x01, 0x00, 0x05]`). Java's lazy reader never reads
/// those regions when the count is zero. No Java writer emits them.
#[test]
...
/// Risk pinned: the metadata's DECLARED data length, not the buffer length, locates the value
/// region. Java reads the value at offset 8 here. A `size_in_bytes` that reported the whole
/// buffer length misreads every concatenated variant that carries trailing dictionary slack.
#[test]
...
// Every constant is the exact serialized output of iceberg 1.10.0. Large fixtures pin
// (CRC-32, length, first 64 bytes). `flate2::Crc` computes the same CRC-32 as `java.util.zip`.
...
/// Risk pinned (the write-side core): every fixture in the table above serializes byte-for-byte
/// to Java 1.10.0, and re-serializes byte-identically from its parsed form. One wrong header bit
/// or payload byte silently corrupts tables for every other engine.
#[test]
...
/// Risk pinned: metadata building mirrors `Variants.metadata` exactly. It keeps INSERTION order,
/// never re-sorting and never deduping. It sets the sorted flag only for strictly ascending
/// `compareTo` input, where a byte-order comparator sets it wrongly for
/// `w_metadata_utf16_sorted`. It escalates the offset size at 255 to 256 data bytes.
#[test]
...
/// Risk pinned (documented divergence): Java 1.10.0 silently corrupts more than 255 names whose
/// data size still selects a 1-byte offset width. Its masked count truncates to 0, so 256 empty
/// names serialize to `01 00 00` and every name is lost. The Rust door rejects it loudly.
#[test]
...
/// Risk pinned (width-selection boundaries): the array count boundary of 255 against 256, and
/// the offset width escalation at dataSize 65535 against 65536. A transposed is-large bit or an
/// off-by-one width threshold changes the pinned bytes.
#[test]
...
/// Risk pinned (the UTF-16 trap, write direction): on-disk field order is Java `String.compareTo`
/// over UTF-16 code units, so U+10000 sorts BEFORE U+FFFF even though its UTF-8 bytes sort after.
/// The puts arrive in the opposite order, and a byte-order sort emits the fields transposed.
#[test]
...
/// Risk pinned: `fieldIdSize = sizeOf(metadata.dictionarySize())` reads the dictionary SIZE, not
/// the largest id used. A 256-name dictionary writes 2-byte field ids (id 255 = `ff 00`) even
/// though 255 fits one byte.
#[test]
...
/// Risk pinned: object to array to object serializes byte-for-byte to Java 1.10.0. It exercises
/// recursive size computation and offset assignment across all three container levels.
#[test]
...
fn test_write_buffer_too_small_or_bad_offset_rejects() {
    ...
    // Containers door the WHOLE span up front, so a failed write leaves the buffer untouched.
    // Without the door the per-write bounds checks still error, but only after they mutate the
    // caller's buffer. Removing the door survives every `is_err` pin.
    let mut array = VariantArray::new();
...
/// Risk pinned (the object-side offset-width escalation, which the array pair cannot catch):
/// an object with dataSize 65535 keeps 2-byte offsets, and 65536 escalates to 3-byte.
/// `offsetSize = sizeOf(dataSize)` also feeds `objectHeader`, so an off-by-one flips the
/// object header byte and every offset-list byte. Pinned against Java 1.10.0.
#[test]
...
/// Risk pinned (the only fixture that forces offsetSize 4): an array with dataSize 0xFFFFFF
/// keeps 3-byte offsets, and 0x1000000 escalates to 4-byte. Java serializes both legally, so
/// the Rust doors must not reject the wide one. The `0b11` width bits and the 4-byte
/// little-endian offset write must match Java 1.10.0 exactly.
#[test]
...
/// Risk pinned (duplicate-name id resolution): a dictionary may hold duplicate names. Java's
/// unsorted `SerializedMetadata.id` linear-scans from index 0, so the FIRST duplicate wins and
/// its writer emits id 0. Rust must resolve identically. A parsed object that references the
/// second duplicate re-serializes here with id 0, the documented canonicalization divergence.
/// Java copies the original buffer verbatim and keeps id 1. The decoded value is unchanged.
#[test]
...
// ===== shredding overlay: Java 1.10.0 byte-exact fixtures ==================================
// Every overlay constant is the exact output of iceberg-core 1.10.0 `ShreddedObject.writeTo`.
// The probe lines quoted per test come from the same generator run.
...
/// Risk pinned (the core merge semantics): a `put` overrides the backing's field, a new `put`
/// merges in, `remove` excludes one, and the header, ids, and offsets recompute over the MERGED
/// set. A wrong merge silently rewrites object content for every shredded write.
#[test]
...
/// Risk pinned: untouched unshredded fields serialize as VERBATIM slices of the original buffer
/// (Java `SerializedObject.sliceValue`). A re-encode through the canonicalizing writer shrinks the
/// long-form string to the short form, and diverges from Java on every non-canonical input.
#[test]
...
/// Risk pinned: Java's remove-then-put contract is inconsistent by construction. `put` does not
/// clear `removedFields`, so the views exclude the name while serialization INCLUDES the new
/// value. Mirroring only the views, or only the bytes, silently diverges.
#[test]
...
/// Risk pinned: duplicate resolved field names reject at SERIALIZATION time, unless the name is
/// replaced or removed, which takes both occurrences off the unshredded side. An eager
/// constructor-time rejection over-rejects.
#[test]
...
// The r* hex lines quoted below are verbatim output of a Java 1.10.0 probe program.

/// Risk pinned (slice-range math): the backing stores fields a/b/c in name order but places their
/// data c-first at widths 2/7/5. The sorted-distinct-offsets scheme gives each untouched field its
/// true span. An adjacent-field subtraction or an off-by-one range copies the WRONG bytes.
#[test]
...
/// Risk pinned (documented divergence): a backing carries a MALFORMED untouched field. Lazy
/// Java slices it blind, so `writeTo` succeeds and copies the bad bytes verbatim. Only a later
/// `get` throws. The eager Rust door rejects at `over_serialized_object`, both for the
/// untouched field and for the replaced-field twin. The rejection is loud, never silent.
#[test]
...
/// Risk pinned: the constructed (non-serialized) backing branch materializes untouched fields
/// consistently for size and for write. Java 1.10.0's FIRST serialization of this shape is
/// corrupt, and re-reading it throws. The pinned bytes are Java's self-healed second
/// serialization, which equals what this port emits directly.
#[test]
...
/// lying-sorted dictionary `size_in_bytes` succeeds and `write_to` fails with Java's
/// `checkState` message.
#[test]
...
/// Risk pinned: the traversal ORDER must match Java exactly, with object fields in stored order,
/// hooks around each, array elements by index, and results threaded post-order. A reordering
/// silently changes every traversal-built artifact, and shredded parquet writers build on this.
#[test]
...
/// Risk pinned: Java recurses into the NAME LOOKUP `object.get(fieldName)`, and an unfindable
/// field in a non-name-sorted object throws. The Rust driver fails loud with a named error, and
/// the after-hook still runs for the failing field before the error propagates.
#[test]
...
/// Risk pinned: the visit recursion shares the parser's depth budget. A 130-deep bomb errors
/// instead of overflowing the stack, and a tree at the parser's maximum depth stays visitable, so
/// the two accepted sets remain equal.
#[test]

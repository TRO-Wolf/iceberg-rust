...
/// | # | rule |
/// |---|---|
/// | 1 | Equality deletes are never file-scoped. |
/// | 2 | `referenced_data_file` wins when set. Spark V2 writers never set it. |
/// | 3 | Else equal `file_path` bounds. Java `PositionDeleteWriter` uses this leg only. |
///
/// Absent, unequal, or truncated bounds stay partition-scoped.
...
impl PopulatedDeleteFileIndex {
    ...
    /// | kind | bucket |
    /// |---|---|
    /// | empty-partition equality | `global_equality_deletes` |
    /// | file-scoped position ([`referenced_data_file_location`]) | `pos_deletes_by_path` |
    /// | other | content-type partition map |
    ///
    /// `refuse_shadowed_deletes` re-derives this keying. Change it here and that guard diverges.

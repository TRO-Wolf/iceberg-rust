...
| `parquet_unknown.rs` | the `unknown` (v3) write seam: `writer_arrow_schema` drops unknown-typed fields (top level and nested struct fields) from the Arrow schema so the file carries no Parquet column for them, and `project_batch` drops the matching Arrow `Null` columns from each incoming batch; a batch missing a written column fails loud. Refuses at build, with `FeatureUnsupported`, the shapes Java `TypeToMessageType` / parquet-java refuse: a struct whose every field is unknown (`Cannot write struct '<col>': every field is unknown, and Parquet refuses an empty group`), and unknown directly under a list element, map key or map value (`Cannot convert element\|key\|value Parquet: unknown (column '<col>.<role>')`); an unknown deeper under a list or map is refused too (`Writing an unknown field under the <role> of '<col>' is not supported`), since the omission descends only through structs |
| `parquet_footer_tests.rs`, `parquet_writer_unsupported_tests.rs`, `parquet_writer_unknown_tests.rs` | the `#[cfg(test)]` cells of the three above |

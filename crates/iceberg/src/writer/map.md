...
| `file_writer/orc_writer.rs` | ORC data-file IO (`OrcWriterBuilder`/`OrcWriter`): Arrow batch → Iceberg `Literal` rows → ORC stripes, then a hand-written footer so every type carries Java's `iceberg.id` / `iceberg.required` attributes. The `DataFileBuilder` carries record count + file size only (engine landed; interop + GAP_MATRIX flip pending) |

...
| A committed delete file deletes nothing (rows "resurrect"), nothing errors | The delete's `partition_spec_id` differs from the data files' — the read side pairs on `(spec_id, partition)` and the commit only validates arity/types, so a wrong-spec stamp is SILENT when the claimed spec's partition type happens to fit. Pass the spec: `with_partition_spec` on the data / position-delete / equality-delete builders (or a `PartitionKey`, which wins). Contract: ENGINE_CONTRACT §7a; pins in `position_delete_writer.rs::spec_stamp_e2e_test` |
| Every write fails `Partition value is not compatible with partition type` on a table nothing else complains about | Same root cause, loud half: the current spec is UNPARTITIONED with a NON-ZERO id (V2 removing a spec's only field), the writer fell back to `DEFAULT_PARTITION_SPEC_ID` (0), and spec 0 is partitioned. Configure the spec |

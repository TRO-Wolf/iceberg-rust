...
| `base_writer/equality_delete_writer.rs` | equality-delete files (equality ids → projected schema); `EqualityDeleteWriterConfig::new` refuses an `unknown`-typed equality id with `FeatureUnsupported` naming the field (row R91) |
...
| `base_writer/deletion_vector_writer.rs` | deletion vectors (V3 Puffin DVs, Java `BaseDVFileWriter`): accumulate `delete(path, pos, partition)`, `close()` → ONE Puffin file, one `deletion-vector-v1` blob per referenced data file (sorted-path order), `DeleteFile` per `createDV` L145-159; serialization in `../delete_vector.rs` (`serialize_deletion_vector_v1`, byte-identical to Java incl. run containers). **Previous-deletes MERGE hook (Arc-E Inc 2):** `with_previous_deletes(HashMap<path, PreviousDeletes>)` mirrors Java `loadPreviousDeletes`; `close_with_result()` → `DVWriteResult { delete_files, rewritten_delete_files }` (Java `DeleteWriteResult`): previous positions unioned into the new DV, file-scoped (`is_file_scoped` = Java `ContentFileUtil.isFileScoped`) source files returned for `RowDelta.remove_deletes`. Read a committed DV back with `delete_vector::load_delete_vector(&FileIO, &DataFile)` (F-13 U3a) — the public mirror of Java `BaseDeleteLoader.readDV`, and the thing that feeds `with_previous_deletes`. `DVWriteResult` also exposes Java `DeleteWriteResult`'s third member as DERIVED accessors — `referenced_data_files()` / `references_data_files()` (F-13 U3b) — feeding `RowDelta`'s `validateDataFilesExist` arming. Shared-Puffin DML closure lives in `../delete_vector_container.rs` (`close_touched_dv_containers` on current snapshot, `close_touched_dv_containers_at` for a scanned/branch snapshot). **F-22:** one delete-manifest pass returns `legacy_deletes` and `data_sequence_numbers`. **F-23:** the data-manifest walk is skipped when `pending_legacy` is empty and `known_partitions` covers every touched path (`data_sequence_numbers` empty then); otherwise it runs and stops once every wanted path is found. **F-23 r2 (2026-09-03):** the first data manifest is loaded alone and only the tail is buffered (an ordered buffer of 8 issued 8 GETs for a one-manifest hit on any store whose read is not ready on the first poll); with no legacy deletes only the touched paths `known_partitions` missed are wanted, so `data_sequence_numbers` carries just those. `load_legacy_positions_by_path` is one projected parquet read per delete file; optional pre-loaded `ManifestList` (RePark consumes it; F-26: the in-tree MoR DELETE/UPDATE plans also supply `known_partitions` from their scan tasks, so the skip fires in tree too). F-19: `retained_references` is gone; files-exist covers replacement blobs only. Maintenance no longer copies live siblings (`rewrite_siblings_for_dropped_references` deleted). F-18: `delete` resolves the map entry with `get_mut` first and allocates the path only on insert; `is_file_scoped` moved to the sibling `deletion_vector_writer/file_scope.rs`. The container close is SPARK-EQUAL — `RowDelta` removal is keyed by the Java `DeleteFileSet` triple, so one blob of a shared Puffin is replaced on its own and the untouched sibling entry keeps pointing at the old container; the DV merge goes through `with_previous_deletes` rather than re-inserting every previous position. Status: GAP_MATRIX row R114 |
| `file_writer/parquet_writer.rs` | Parquet IO + per-column metrics collection (the bounds the evaluators later prune on); normalizes TOP-LEVEL UTC-alias `timestamptz` batches (`"UTC"`↔`"+00:00"`) to the writer schema (`UTC_TIME_ZONE` = `"UTC"`) metadata-only at the `write` funnel (F-A2-3, values bit-identical). **Refuses a variant-bearing schema at `build()`**, before any bytes reach storage — file-level variant I/O is unimplemented (row R88), and the Iceberg→Arrow conversion of `variant` now SUCCEEDS, so without this the failure would land at `close()` and leave an orphan file. Shares `arrow::schema::variant_path_within` with the reader's guard. **Omits `unknown` fields at the top level and in structs** (row R91, via `file_writer/parquet_unknown.rs`) so the file carries no Parquet column for them, like Java `TypeToMessageType`; refuses at `build()` the shapes Java refuses — a struct whose every field is unknown (empty group) and unknown under a list element, map key or map value |
| `file_writer/parquet_compression.rs` | Parse `write.parquet.compression-codec` / `write.parquet.compression-level` into `parquet::basic::Compression`. Default codec is `zstd`; an unset level maps to `ZstdLevel::try_new(3)` — the Java/Spark zstd default, not parquet-rs's `ZstdLevel::default()` = 1 (F-REWRITE-SIZE-1 step 2). `lz4`/`lz4_raw` map to `LZ4_RAW`. Unknown codec or bad level is `DataInvalid` naming the key and value. Consumed by INSERT (`IcebergWriteExec`), `rewrite_data_files` compaction, partition-key repair, COW/MoR rewrite output (`row_lineage`), and `position_delete_writer_properties_for` (F-WRITE-COMPRESS-1/-2) |
| `file_writer/parquet_footer.rs` | builds the `ArrowWriterOptions` each `ParquetWriterBuilder` prepares once and every file it rolls is written with (F-PARQUET-SIZE-1): injects `iceberg.schema` (Java `Parquet.WriteBuilder.build()` stamps `SchemaParser.toJson(schema)` before constructing the writer) and suppresses the arrow-rs `ARROW:schema` blob that Java never writes, so the footer key-values match a Java file's exactly. The schema JSON is serialized in Java's key order by `java_ordered_schema_json` — schema wrapper (`type`, `schema-id`, `identifier-field-ids`, `fields`), field (`id`, `name`, `required`, `type`, `doc`, `initial-default`, `write-default`), list (`type`, `element-id`, `element`, `element-required`), map (`type`, `key-id`, `key`, `value-id`, `value`, `value-required`) — footer-local, so the global `Type` serde that writes table metadata is untouched; and the parquet schema root is named `table` like parquet-mr, not `arrow_schema` (F-RPD-TARGET-SMALL-1). Also hosts the delete-file `delete-type` key (Java `Parquet.DeleteWriteBuilder`) — `equality_delete_writer_properties[_for]` stamp `equality`; the position-delete property builders stamp `position`. Reads are unaffected: the reader resolves fields from parquet `field_id` metadata, and Spark files never carried `ARROW:schema`. `parquet_footer_tests.rs` is its `#[cfg(test)]` cell |
...
| `file_writer/orc_writer.rs` | ORC data-file IO (`OrcWriterBuilder`/`OrcWriter`): Arrow batch → Iceberg `Literal` rows → ORC stripes, then a hand-written footer so every non-root type carries Java's `iceberg.id` / `iceberg.required` attributes (the root carries field names only, like Java). The `DataFileBuilder` carries record count, file size and split offsets plus the full Java column-metric set under `MetricsConfig`, mirroring the parquet writer (engine landed; interop + GAP_MATRIX flip pending) |
| `file_writer/any_writer.rs` | the format-selection seam over the Parquet / Avro / ORC builders: `AnyFileWriterBuilder::for_format` instantiates by `DataFileFormat`; puffin is refused as a sidecar |
| `write_defaults.rs` | fill missing top-level primitive columns from `write-default` (row R92) + relabel every written column's nested Arrow fields to the Iceberg target type (F-LIST-INSERT-1); called from `DataFileWriter::write` only, which caches the target `ArrowSchemaRef` at `build()`. Extra batch columns are dropped; a write schema that includes the reserved row-lineage pair (`schema_with_row_lineage`) keeps them, which is how v3 rewrite/COW persist stored `_row_id` / `_last_updated_sequence_number` (row R166) |
| `write_defaults_tests.rs` | the `#[cfg(test)]` cell module for `write_defaults.rs` (extracted for the file-size ceiling); shared `pub(crate)` fixtures used by `base_writer/data_file_writer_tests.rs` |
| `base_writer/data_file_writer_tests.rs` | the `#[cfg(test)]` cell module for `base_writer/data_file_writer.rs` (extracted for the file-size ceiling); reuses the `write_defaults_tests.rs` fixtures |
...
Parity gaps live in the GAP_MATRIX (sort-order-aware writing; the Avro data-file
**writer engine** has landed in `file_writer/avro_writer.rs`, and the ORC data-file **writer
engine** has landed in `file_writer/orc_writer.rs` — the GAP_MATRIX flips wait on the interop
round-trips). Deletion-vector capability status and shared-Puffin DML closure limits live on
GAP_MATRIX row R114.
...
| `FeatureUnsupported` `Cannot write struct '…': every field is unknown` or `Cannot convert element\|key\|value Parquet: unknown` on the first `DataFileWriter::write` | The Iceberg schema carries `unknown` in a shape Java also refuses: a struct left empty once its unknown fields are omitted, or unknown under a list or map. Refusal is at `ParquetWriterBuilder::build`, before bytes land. Top-level and struct-sibling unknown fields write (omitted from the file). |

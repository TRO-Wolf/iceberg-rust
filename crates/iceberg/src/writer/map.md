...
| `write_defaults.rs` | fill missing top-level primitive columns from `write-default` (row R92); called from `DataFileWriter::write` only |
...
interop round-trip). The deletion-vector writer is now ✅ — serialization (D2), commit (D3), and the
previous-deletes merge hook + replacement interop (Arc-E Inc 2) all landed.
...
| A committed POSITION delete deletes nothing (rows "resurrect"), nothing errors | Its `partition_spec_id` differs from the data files' — the read side pairs on `(spec_id, partition)` and the commit only validates arity/types, so a wrong-spec stamp is SILENT when the claimed spec's partition type happens to fit. Pass the spec: `with_partition_spec` on the data / position-delete / equality-delete builders (or a `PartitionKey`, which wins). Contract: ENGINE_CONTRACT §7a; pins in `position_delete_writer.rs::spec_stamp_e2e_test`. Same story for an equality delete carrying a NON-EMPTY tuple — but NOT for a keyless one, see the next row |
...
| Every write fails `Partition value is not compatible with partition type` on a table nothing else complains about | Same root cause as the position-delete row, loud half: the current spec is UNPARTITIONED with a NON-ZERO id (V2 removing a spec's only field), the writer fell back to `DEFAULT_PARTITION_SPEC_ID` (0), and spec 0 is partitioned. Configure the spec |

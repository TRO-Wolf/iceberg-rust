...
| `file_writer/parquet_compression.rs` | Parse `write.parquet.compression-codec` / `write.parquet.compression-level` into `parquet::basic::Compression`. Default codec is `zstd`. `lz4`/`lz4_raw` map to `LZ4_RAW`. Unknown codec or bad level is `DataInvalid` naming the key and value. INSERT (`IcebergWriteExec`) consumes this; other writers are residue |

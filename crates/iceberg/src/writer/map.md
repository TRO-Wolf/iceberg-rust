...
| `file_writer/parquet_footer.rs` | builds the `ArrowWriterOptions` every parquet data/delete file is written with (F-PARQUET-SIZE-1): injects `iceberg.schema` (Java `Parquet.WriteBuilder.build()` stamps `SchemaParser.toJson(schema)` before constructing the writer) and suppresses the arrow-rs `ARROW:schema` blob that Java never writes, so the footer key-values match a Java file's exactly. Reads are unaffected: the reader resolves fields from parquet `field_id` metadata, and Spark files never carried `ARROW:schema`. `parquet_footer_tests.rs` is its `#[cfg(test)]` cell |

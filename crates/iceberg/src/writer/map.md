...
| `file_writer/parquet_footer.rs` | builds the `ArrowWriterOptions` each `ParquetWriterBuilder` prepares once and every file it rolls is written with (F-PARQUET-SIZE-1): injects `iceberg.schema` (Java `Parquet.WriteBuilder.build()` stamps `SchemaParser.toJson(schema)` before constructing the writer) and suppresses the arrow-rs `ARROW:schema` blob that Java never writes, so the footer key-values match a Java file's exactly. Also hosts the delete-file `delete-type` key (Java `Parquet.DeleteWriteBuilder`) — `equality_delete_writer_properties[_for]` stamp `equality`; the position-delete property builders stamp `position`. Reads are unaffected: the reader resolves fields from parquet `field_id` metadata, and Spark files never carried `ARROW:schema`. `parquet_footer_tests.rs` is its `#[cfg(test)]` cell |

...
//! one `Delete` snapshot.
//!
//! **Known inherited divergence: by-path delete plus a PARTIAL filter match on the same file.** Java's
//! `ManifestFilterManager.manifestHasDeletedFiles` marks a file for deletion by path before the metrics
//! check, and a marked file skips the classification entirely. Java therefore deletes it.
//! [`SnapshotProducer::resolve_filter_deletes`] has no such short-circuit, so it raises the
//! "some, but not all, rows match" error instead.
//!
//! The divergence is pre-existing and inherited. The same helper drives
//! `OverwriteFiles.overwriteByRowFilter`. It is fail-safe: the fork refuses the commit and never wrongly
//! deletes data. The fix belongs in `snapshot.rs` and is deferred. The test
//! `test_delete_from_row_filter_bypath_and_partial_match_diverges_failsafe` pins it.
//!
//! **Case sensitivity.** [`DeleteFilesAction::case_sensitive`] threads the Java
//! `MergingSnapshotProducer.caseSensitive` flag into the row-filter binding. It defaults to `true`, the
//! Java default, so the filter resolves column names case-sensitively.
//!

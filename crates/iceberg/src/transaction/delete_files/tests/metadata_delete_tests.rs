...
use crate::spec::{
    ...
    NestedField, Operation, PrimitiveType, Schema, Struct, StructType, Transform, Type,
...
async fn make_oracle_table_in_catalog(
    ...
    make_table_in_catalog(catalog, format_version, spec, oracle_schema()).await
}
...
async fn can_delete_using_metadata_string_lt_bmp_upper_below_supplementary() {
    let catalog = new_memory_catalog().await;
    let table = make_oracle_table_in_catalog(&catalog, FormatVersion::V2, None).await;
    let file = write_oracle_data_file(&table, "test/unicode1.parquet", &[(1, "x", "\u{FFFF}")]).await;
    let table = append_files(&catalog, &table, vec![file]).await;
...
async fn can_delete_using_metadata_string_lt_mixed_bounds_below_supplementary() {
    let catalog = new_memory_catalog().await;
    let table = make_oracle_table_in_catalog(&catalog, FormatVersion::V2, None).await;
    let file = write_oracle_data_file(
        &table,
        "test/unicode2.parquet",
        &[(1, "x", "\u{FFFF}"), (2, "x", "\u{10000}")],
    )
    .await;
    let table = append_files(&catalog, &table, vec![file]).await;
...
async fn can_delete_using_metadata_string_gt_bmp_upper_above_pua() {
    let catalog = new_memory_catalog().await;
    let table = make_oracle_table_in_catalog(&catalog, FormatVersion::V2, None).await;
    let file = write_oracle_data_file(&table, "test/unicode3.parquet", &[(1, "x", "\u{FFFF}")]).await;
    let table = append_files(&catalog, &table, vec![file]).await;
...
async fn can_delete_using_metadata_starts_with_supplementary_prefix_is_vacuous() {
    ...
    let file = write_oracle_data_file(
        &table,
        "test/unicode6.parquet",
        &[(1, "x", "\u{E000}"), (2, "x", "\u{FFFF}")],
    )
    .await;
    let table = append_files(&catalog, &table, vec![file]).await;
...
async fn can_delete_using_metadata_not_starts_with_order_boundary() {
    ...
    let file = write_oracle_data_file(
        &table,
        "test/unicode8.parquet",
        &[(1, "x", "\u{E000}"), (2, "x", "\u{10000}z")],
    )
    .await;
    let table = append_files(&catalog, &table, vec![file]).await;

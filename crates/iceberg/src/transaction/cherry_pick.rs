...
//!
//! 1. **Fast-forward, which takes precedence over replay.** When the staged snapshot's PARENT is the
//!    current `main` head, or both are null, `main` moves to the staged snapshot AS-IS. No new
//!    snapshot is produced and no files are replayed. This runs BEFORE the replay paths, so an
//!    append or a replace-partitions overwrite whose parent is head also fast-forwards.
//! 2. **APPEND replay.** The staged snapshot's added data files are replayed into a NEW `Append`
//!    snapshot on `main`, tagged with `source-snapshot-id` and, when the staged snapshot carries a
//!    non-empty `wap.id`, `published-wap-id`.
//! 3. **OVERWRITE with `replace-partitions=true` replay.** Added AND removed data files are replayed
//!    into a NEW `Overwrite` snapshot. The removed files drop by path, so they must still be live.
//!    The replaced partitions are tracked so [`CherryPickAction::validate`] can reject a concurrent
//!    change in them.
//!
...
//!
//! **WAP-path publish dedup.** Two dedup paths run in [`CherryPickAction::validate`], in Java's
//! order: the ancestry path first, the WAP-id path last. When both apply, the ANCESTRY error fires.
//! The WAP-id check walks the current ancestry and rejects when the picked snapshot's non-empty
//! `wap.id` already appears there. It compares against each ancestor's OWN `wap.id` AND its
//! `published-wap-id`. BOTH arms matter: a replay publish stamps `published-wap-id`, while a
//! fast-forward publish keeps only the staged snapshot's own `wap.id`. This prevents a duplicate WAP
//! publish double-applying the same audited change.
//!

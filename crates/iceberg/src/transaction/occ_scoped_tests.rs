...
pub(crate) fn x_equals(value: i64) -> Predicate {
...
pub(crate) async fn live_file_paths(table: &Table) -> HashSet<String> {
    let snapshot = table
        .metadata()
        .current_snapshot()
...
pub(crate) fn snapshot_len(table: &Table) -> usize {
...
#[tokio::test]
async fn row_delta_not_filter_disjoint_partition_append_conflicts() {
    let catalog = new_memory_catalog().await;
    let table = make_v2_minimal_table_in_catalog(&catalog).await;
    let table = append_files(&catalog, &table, vec![data_file("test/base.parquet", 1)]).await;
    ...
    let err = tx
        .commit(&catalog)
        .await
        .expect_err("a concurrent append satisfying NOT (x = 1) must conflict under that filter");
    assert_eq!(err.kind(), ErrorKind::DataInvalid);
    assert!(!err.retryable());
    assert!(err.message().contains("test/other-part.parquet"));
}

#[tokio::test]
async fn row_delta_not_filter_matching_partition_append_commits() {
    let catalog = new_memory_catalog().await;
    let table = make_v2_minimal_table_in_catalog(&catalog).await;
    let table = append_files(&catalog, &table, vec![data_file("test/base.parquet", 1)]).await;
    ...
    let table = tx
        .commit(&catalog)
        .await
        .expect("a concurrent append excluded by NOT (x = 1) must not conflict under that filter");
    let live = live_file_paths(&table).await;
    assert!(live.contains("test/op.parquet"));
    assert!(live.contains("test/same-part.parquet"));
}
...
#[tokio::test]
async fn row_delta_compound_not_filter_commits_when_both_arms_excluded() {
    let catalog = new_memory_catalog().await;
    let table = make_v2_minimal_table_in_catalog(&catalog).await;
    let table = append_files(&catalog, &table, vec![data_file("test/base.parquet", 1)]).await;

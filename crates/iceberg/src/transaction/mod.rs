...
mod sort_order;
pub use sort_order::ReplaceSortOrderAction;
mod update_location;
...
impl Transaction {
    ...
    /// retried. It is RECONCILED by re-reading the catalog. Finding the attempt in reloaded
    /// metadata is SUCCESS. Otherwise the original error surfaces: the in-flight request may
    /// still land. A commit with no snapshot is not reconciled.

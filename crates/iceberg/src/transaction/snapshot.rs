...
impl<'a> SnapshotProducer<'a> {
    ...
    /// Resolve the LIVE data files this overwrite removes BY ROW PREDICATE, returning every file
    /// the `predicate` STRICTLY matches. Java `ManifestFilterManager.manifestHasDeletedFiles` plus
    /// `PartitionAndMetricsEvaluator`. The resolved files feed the same [`process_deletes`] path, so
    /// they drop in the one `Operation::Overwrite` snapshot.
    ///
    /// Each file's predicate is first reduced to its per-partition RESIDUAL, then the strict and
    /// inclusive metrics evaluators run on THAT residual. This is what makes a partition-column
    /// predicate delete a file with no bounds for that column: the residual is `alwaysTrue`, which
    /// the strict evaluator satisfies. Running the metrics on the FULL predicate would wrongly call
    /// such a file a partial match.
    ///
    /// Decision tree per live data file:
    ///
    /// 1. **KEEP** when [`InclusiveMetricsEvaluator::eval`] says no rows can match.
    /// 2. **DELETE** when [`StrictMetricsEvaluator::eval`] says all rows must match.
    /// 3. **ERROR** on a partial match, with Java's verbatim non-retryable message.
    ///
    /// With no partition fields the residual is the whole filter, so an `alwaysTrue` predicate
    /// deletes every live data file.
    ///
    /// # Notes
    ///
    /// The predicate binds with the caller's `case_sensitive` flag; the actions default it to
    /// `true`. The residual evaluator caches per partition-spec id, because manifests can carry
    /// different spec ids.
...
///
/// - **Sequence number:** the delete's data sequence number must be `>= starting_sequence_number`.
///   An absent entry sequence number counts as applicable, the conservative reading.
/// - **Global equality deletes:** an equality delete with an empty partition applies to ANY data
///   file, subject to the sequence test.
/// - **Partition match:** otherwise the delete needs the same `partition_spec_id` and an equal
///   partition tuple.
/// - **Path-scoped position deletes:** a `referenced_data_file` must equal the data file's path.
...
///
/// The Rust port of Java `MergingSnapshotProducer.validateAddedDataFiles`. It enumerates the
/// concurrently-added DATA files through [`added_data_files_after`] and rejects the FIRST file whose
/// metrics permit a match. The per-file test is [`InclusiveMetricsEvaluator`].
///
/// `current` is the REFRESHED base. `effective_start` is exclusive, and `None` inspects from the
/// root. A `None` `conflict_filter` binds `AlwaysTrue`, so ANY concurrently added DATA file
/// conflicts, the most conservative serializable check. The actions default `case_sensitive` to
/// `true`.
///

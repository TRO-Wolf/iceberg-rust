...
//!
//! 1. **Short-circuit**: return the list unchanged when merging is disabled or the list is empty.
//! 2. **Group by partition spec id, REVERSE-sorted.** Manifests of different spec ids are NEVER
//!    merged together, so a merged manifest carries exactly one spec id.
//! 3. **Bin-pack each group** from the END, so the under-filled bin is the FIRST one and gets merged
//!    on the next append. A bin of one is kept as-is. A bin holding this commit's new manifest is
//!    kept as-is while it is below `minCountToMerge`. That gate applies ONLY to that bin, so a large
//!    old manifest cannot block merging older groups. Every other bin merges into one manifest.
//! 4. **`createManifest`**, per entry of each merged manifest:
//!    - `DELETED` is kept only when the entry belongs to this snapshot. Older tombstones are
//!      SUPPRESSED.
//!    - `ADDED` from this snapshot stays `Added` and re-inherits the new sequence number.
//!    - Everything else is written as `Existing` with its provenance intact. RE-STAMPING a carried
//!      entry's provenance is the number one silent-corruption class.
//!
...
//!
//! - **Delete-manifest merging is deferred.** This port carries every DELETE manifest forward
//!   UNCHANGED, appended after the merged data manifests. A merge_append never adds a delete file,
//!   so carrying them forward is correctness-safe.
//! - **The retry cache, `cleanUncommitted` and `replacedManifestsCount` are not ported.** The Rust
//!   producer recomputes the merge from the refreshed base on every attempt, and does no orphan-file
//!   cleanup. That gap is fork-wide, not merge-specific.
//! - **`appendManifest` is not ported.** `fast_append` has no `add_manifest` either, so this is a
//!   parity gap, not a regression.
//! - **No `manifests-created`, `-kept` or `-replaced` summary keys.** A merge_append's summary has
//!   the same shape as a fast_append's.
//! - **`scanManifestsWith(ExecutorService)` parallelism is not ported.** The Rust path is sequential
//!   async.

...
//!
//! - **Clustered rewrite** ([`RewriteManifestsAction::cluster_by`]): every DATA manifest matching
//!   [`RewriteManifestsAction::rewrite_if`] is re-read and its live entries are re-grouped by
//!   `(cluster_key, partition_spec_id)`. **The load-bearing invariant: each re-grouped entry keeps
//!   its original snapshot id and both sequence numbers, and its status becomes `Existing`.**
//!   Re-stamping provenance here is the number one silent-corruption class. It breaks merge-on-read
//!   delete application and incremental scans.
//! - **Explicit replacement** ([`RewriteManifestsAction::add_manifest`] and
//!   [`RewriteManifestsAction::delete_manifest`]): swap a current manifest for an
...
//!
//! - **String cluster key.** Java clusters on `Object` equality. This port takes
//!   `Fn(&DataFile) -> String`, which covers the practical keys and gives a `Hash + Eq` map key
//!   without an `Any` downcast.
//! - **Estimated-length size rolling.** [`crate::spec::ManifestWriter`] buffers its entries and
//!   exposes no incremental on-disk length, so this port rolls on an estimate built from the source
//!   manifest's average per-entry size. That shifts the roll POINTS only. Every live entry is still
//!   written exactly once with its provenance preserved. See [`ClusterWriters`].
//! - **V1 `add_manifest` is unsupported.** On V2 and above the manifest inherits the new snapshot
//!   id. On V1 Java copies the manifest and re-stamps every entry, a path this action defers and
//!   rejects with [`ErrorKind::FeatureUnsupported`].
//! - **No-current-snapshot is a clean error, not a panic.** Java NPEs on an empty table.
//! - **`scanManifestsWith(ExecutorService)` is not ported.** The Rust path is sequential async.
//! - **`requiresRewrite` statefulness is dropped.** The Rust action holds no cross-attempt state and
//!   recomputes from the refreshed base. The Java cache is a retry optimization, not a behaviour.

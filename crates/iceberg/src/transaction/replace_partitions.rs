...
//! - `operation()` is always [`Operation::Overwrite`].
//! - the constructor sets `SnapshotSummary.REPLACE_PARTITIONS_PROP` to `"true"`.
//! - `addFile(file)` drops `file.partition()` in `file.specId()`, then adds the file.
//! - An unpartitioned data spec is a FULL replace. Java `apply()` adds `deleteByRowFilter(alwaysTrue)`,
//!   which removes every live data file UNDER EVERY SPEC. See
//!   [`ReplacePartitionsAction::is_full_table_replace`]. On a table with one unpartitioned spec this looks
//!   like dropping the single empty partition. On a table whose spec evolved it does not, so the branch is
//!   explicit here instead of emergent.
//! - A replaced partition with no existing files is a pure add. Java's `failMissingDeletePaths` guards
//!   path deletes only, never partition drops.
//!
...
//! summary property.
//!
//! **Concurrent-commit conflict validation.** Two opt-in and INDEPENDENT flags mirror Java
//! `BaseReplacePartitions.validate`. Each one evaluates over the scope [`ConflictScope`] resolves: the
//! replaced `(spec_id, partition)` tuples, or the spec-agnostic `alwaysTrue` scope over every file when the
//! added files' spec is unpartitioned. `OverwriteFiles` and `RowDelta` check per data file instead.
//! - [`ReplacePartitionsAction::validate_no_conflicting_data`] rejects a concurrent snapshot that ADDED
//!   data to a replaced partition.
//! - [`ReplacePartitionsAction::validate_no_conflicting_deletes`] rejects a concurrent snapshot that, in a
//!   replaced partition, DELETED a data file or ADDED a delete file.
//!

...
| File | What it does |
|---|---|
| `rewrite_data_files.rs` | Bin-pack compaction. Java `RewriteDataFiles`. Plans candidates, rewrites live rows, commits once by default and per batch under partial progress through `RewriteFiles`. |
...
| Ratio fires but the parquet delete file stays | Expected since F-RDF-COW-BYTES-1: `plan_dv_removal` is Puffin-only (`is_deletion_vector`), so a file-scoped parquet delete stays live after its referenced file is rewritten, as Java's `danglingDVs` does. `remove_dangling_deletes` defaults off and uses a seq `<` clause that often keeps the same-seq delete. |
...
| A shared partition-scoped delete vanishes when one file is rewritten | `plan_dv_removal` drops only Puffin DVs (`is_deletion_vector`); a parquet delete is never dropped by the rewrite commit. Pin: `test_partition_scoped_delete_survives_partial_rewrite`. |

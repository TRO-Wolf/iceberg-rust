...
| `rewrite_data_files_format_tests.rs` | D-4 site-4 pins: compaction output honors `write.format.default` (ORC/Avro end to end incl. the sorted arm and the legacy run-sort arm, rollover, parquet inputs under an ORC default, V3 ORC lineage carry), the Parquet arm keeps today's construction (dictionary on for low-cardinality columns, fallback off for the unique column), garbage/`puffin` values refuse typed, and sort-spill files stay parquet and vanish on success and on failure. |
| `rewrite_data_files_codec_tests.rs` | D-4 site-4 pins: V-001a/b parquet codec ZSTD default plus UNCOMPRESSED honour, V-002 ORC field-2 bounds, V-003a/b ORC none-codec plus bad-stripe-size refusal, and the metrics/property sweep pins. |

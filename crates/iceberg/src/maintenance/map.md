...
| `rewrite_data_files_dv.rs` | Drop file-scoped deletes whose referenced data file this rewrite removes. DVs plus bounds-only parquet position deletes. Rewrite Puffin siblings. |
...
| Drop deletes that targeted a rewritten data file | `rewrite_data_files_dv.rs::plan_dv_removal` (DVs and bounds-only parquet). Not the opt-in dangling-delete sub-action. |
...
| A two-path parquet position delete rewrites both files | Unequal bounds are not file-scoped. Java `isFileScoped` is false. Pin: `test_two_path_parquet_pos_delete_does_not_fire_ratio`. |

...
| `rewrite_data_files_plan.rs` | Candidate and group predicates. Java `BinPackRewriteFilePlanner`. Size band, `tooManyDeletes`, `tooHighDeleteRatio`, `rewrite_all` bypass, output-spec grouping, `RewriteJobOrder`, commit batches. |
...
| `rewrite_data_files_options_tests.rs` | F-RDF-OPTIONS-1 pins: one-commit default, partial progress, `rewrite_all`, `output_spec_id`, job order, concurrency, Java float rendering. |

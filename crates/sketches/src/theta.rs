...
//! Theta sketch update form and CompactSketch v3 serialization, byte-compatible with Apache
//! DataSketches `org.apache.datasketches.theta`.
//!
//! The byte format is the payload of Iceberg's `apache-datasketches-theta-v1` Puffin blob.
//! [`ThetaSketch`] ports `HeapQuickSelectSketch`, so the retained hash set and `theta` match Java
//! to the bit. [`ThetaSketch::serialize_compact`] emits `HeapCompactSketch.toByteArray`.
//!
//! Format facts from `theta/PreambleUtil` and `Family`:
...
//! - A compact sketch zeroes bytes 3 and 4, which are update-only state.
...
/// The DataSketches `2^63` literal. It equals [`MAX_THETA`] `as f64` to the bit.
...
/// Low-5-flag-bits pattern (26) that DataSketches reads as a legacy single-item compact sketch.
...
/// The default lg nominal entries of Iceberg's `theta_sketch_agg`: 4096 nominal, lgK 12.
pub const DEFAULT_LG_NOMINAL_LONGS: u32 = 12;
/// The DataSketches default lg resize factor as an integer (`ResizeFactor.X8.lg() == 3`).
...
/// A theta sketch in mutable form, retaining a bounded hash set. Mirrors Java
/// `HeapQuickSelectSketch`: the table grows to `2^(lg_nominal_longs + 1)`, then quickselect
/// lowers `theta` on rebuild.
#[derive(Debug, Clone)]
...
impl ThetaSketch {
    ...
    /// Builds an empty sketch. `lg_nominal_longs` is clamped to the legal range `4..=26`.
    ...
    /// Updates the sketch with a `u64`, hashed through the long-array path.
    pub fn update_u64(&mut self, value: u64) {
        let full = hash_long(value, self.seed);
        self.update_hash(full[0] >> 1);
    }

    /// Updates the sketch with a `u64` through the byte path. Same result as [`Self::update_u64`].
    pub fn update_long_bytes(&mut self, value: u64) {
        // Java hashes `long[]{v}` and `LE8(v)` alike, so one path serves both entry points.
        self.update_u64(value);
    }

    /// Updates the sketch with a byte string. Empty input is ignored, as Java ignores it.
    ...
    /// Open-addressing search-or-insert. `Some(index)` on a duplicate, `None` after an insert.
    ...
    /// Lowers `theta` to the (nominal+1)-th smallest hash and rebuilds, keeping hashes below it.
    ...
    /// Parses a compact theta sketch and verifies its seed hash against this sketch's seed. It
    /// exists so a caller with a configured seed validates compatibility in one call.
...
pub struct CompactThetaSketch {
    /// The retained hashes in wire order. An unordered blob keeps its order verbatim.
    pub hashes: Vec<u64>,
    /// The theta value as a `long`.
    pub theta: i64,
    /// Whether the sketch is empty.
    pub is_empty: bool,
    /// Whether the `ORDERED` flag was set. An unordered sketch is accepted verbatim.
    pub is_ordered: bool,
}
...
impl CompactThetaSketch {
    ...
    ///
    /// # Errors
    /// A malformed preamble, an unsupported version or family, or a truncated payload.
    pub fn deserialize(bytes: &[u8]) -> SketchResult<Self> {
        Self::parse(bytes, None)
    }

    /// Deserializes a compact theta sketch and checks its seed hash against `seed`.
    ///
    /// # Errors
    /// As [`Self::deserialize`], plus [`SketchError::SeedHashMismatch`] on a foreign seed.
    pub fn deserialize_with_seed(bytes: &[u8], seed: u64) -> SketchResult<Self> {
        Self::parse(bytes, Some(seed))
    }

    /// Core parser. It validates the preamble at every step before it reads the body.
    ///
    /// Java `heapify` accepts serial-versions 1, 2, and 3. This reader is deliberately v3-only:
    /// 1 and 2 are pre-0.10 layouts no Iceberg engine emits, and a loud rejection beats decoding
    /// a layout this crate does not have. Port `ForwardCompatibility` if one is ever needed.
    fn parse(bytes: &[u8], seed: Option<u64>) -> SketchResult<Self> {
        ...
        // Java accepts either the explicit SINGLEITEM bit or a legacy 1-long preamble whose low
        // 5 flag bits are exactly 26.
        ...
        // Java reads curCount from bytes 8-11 only when pre_longs > 1. Reading it on a 1-long
        // preamble that is neither empty nor single over-reads and panics.
...
/// Serializes the compact byte form from explicit parts. It is the single serialization path.
///
/// `hashes` must be ascending-sorted. `is_empty` forces the empty form whatever `hashes` holds.
...
/// `setHashTableThreshold`: `floor(fraction * 2^lgArr)`. Both update families call it.
...
/// `HashOperations.getStride`: the odd double-hashing stride `2 * ((hash >>> lgArr) & 127) + 1`.
pub(crate) fn get_stride(hash: u64, lg_arr_longs: u32) -> u64 {
    2 * ((hash >> lg_arr_longs) & 127) + 1
}

/// `HashOperations.hashSearchOrInsert`: `Some(index)` on a duplicate, `None` after an insert.
...
/// `HashOperations.hashArrayInsert`: re-insert every non-zero source entry below `theta`.
...
/// `QuickSelect.selectExcludingZeros`: the (nominal+1)-th smallest non-zero hash. This sorts the
/// non-zero entries rather than doing Java's in-place quickselect. The result is the same.
fn select_excluding_zeros(table: &[u64], nominal: usize) -> i64 {
    ...
    // The (nominal+1)-th smallest sits at index `nominal`, and `curCount > threshold`.
...
mod tests {
    ...
    /// Every `*_HEX` came from `Gen*.java` against `datasketches-java-3.3.0` with seed 9001.
    ...
    // Estimation-breadth fixture: lgK=8, 5000 distinct longs `1_000_000 + i*7`. Seed-independent,
    // and it forces deep resize plus repeated quickselect.
    ...
    fn test_estimation_mode_serialization_matches_java_bytes() {
        // Risk: estimation mode. The retained set and theta must reproduce Java's quickselect.
    ...
    fn test_estimation_mode_estimate_within_theoretical_bounds() {
        // Risk: wrong estimator math gives a wildly off NDV. The relative standard error at
        // k=16 is about 0.25, so 1000 distinct inputs must land inside 3 sigma.
        let mut sketch = ThetaSketch::with_lg_nominal_longs(4, DEFAULT_UPDATE_SEED);
        for value in 0..1000u64 {
            sketch.update_u64(value);
        }
    ...
    fn test_starting_sub_multiple_and_threshold_match_java() {
        // Risk: a wrong initial table size changes whether a build resizes or quickselects,
        // which diverges the retained set.
    ...
    fn test_estimation_mode_lgk8_5000_matches_java_bytes_and_estimate() {
        // Risk: one lgK=4 fixture misses a quickselect or resize bug another lgK would catch.
    ...
    fn test_estimation_default_lgk12_100k_contract_matches_java() {
        // Risk: the default lgK 12 entering estimation with 100k distinct values. The retained
        // set, theta, and estimate are the cross-engine contract.
    ...
    fn test_estimate_is_bit_exact_with_java_not_just_close() {
        // Risk: `count / (theta / MAX)` rounds one ULP away from `count * (MAX / theta)` here.
        // This pins the exact f64 bits, which the 1e-6 assertion elsewhere cannot.
        let est = CompactThetaSketch::deserialize_with_seed(
            &hex_to_bytes(EST1000_LGK4_HEX),
            DEFAULT_UPDATE_SEED,
        )
    ...
    fn test_unordered_compact_blob_is_accepted_like_java() {
        // Risk: an unordered compact sketch has the ORDERED flag clear and must be accepted
        // verbatim. This is the EXACT10 set in hash-table iteration order, so it is not sorted.
    ...
    fn test_serial_version_1_and_2_rejected_loudly() {
        // Java accepts serial-versions 1 and 2. This v3-only reader must reject them loudly.
    ...
    fn test_prelongs1_nonsingle_nonempty_reads_as_zero_entries_like_java() {
        // Risk: a preLongs=1 blob that is neither EMPTY nor SINGLEITEM is exactly 8 bytes. The
        // reader indexed `bytes[8..12]` and panicked. Java reads curCount=0, so this returns a
        // 0-entry sketch.
    ...
    fn test_prelongs1_legacy_single_flag_reads_one_hash_like_java() {
        // Risk: low-5 flag bits 26 mean a legacy single item whose hash sits at byte 8. Without
        // that arm the reader falls through to multi-entry and panics or misreads.
    ...
    fn test_retained_count_allocation_bomb_errors_before_alloc() {
        // Risk: a blob claiming retained = i32::MAX must fail closed with `TruncatedInput`
        // before it allocates 16 GiB. Java trusts curCount and throws `OutOfMemoryError`.
    ...
    fn test_empty_flag_with_nonzero_count_short_circuits_like_java() {
        // Risk: the EMPTY flag beats the retained-count field. A blob with EMPTY set and a
        // non-zero count must read as empty, as Java's heapify does.
    ...
    fn test_corrupt_theta_zero_yields_infinity_like_java() {
        // Risk: theta == 0 is corrupt. Java yields `+inf`, so this must too, without panicking.
        let bytes =
    ...
    fn test_descending_ordered_hashes_accepted_verbatim_like_java() {
        // Risk: an ORDERED-flagged blob whose hashes descend. Java does not re-validate ordering
        // on read, so this accepts it. A future check must be a conscious divergence.
        let bytes =

...
## 18 · Final round (2026-08-08) — F-A/F-B/F-C, and the NAMED RESIDUE

The Falsifier that reviewed §17 **upheld all seven closures** and found further items. This round was
scoped to CONVERGE, not expand: exactly three items were built — the only one that is a demonstrated
silent row-loss path, one FALSE doc claim, and one methodology lesson. Everything else the sweep
turned up is recorded below as residue with its exact mutant string, unbuilt by design. **No
production line changes here either** (F-A is a test, F-B and F-C are prose).

| # | Sev | What | Built | Mutation → result |
|---|---|---|---|---|
| F-A | S2 | `split_fixed_size` (`scan/task.rs`) had an UNPINNED loop bound. On a parent whose `length` IS 1 the mutant makes the walk never run, so `split` returns an EMPTY Vec and `scan::mod`'s `split_tasks.extend(task.split(split_size)?)` DROPS the file: `plan_files` returns it, `plan_tasks` reads ZERO rows, nothing errors — the same silent-total-row-loss class as branch (1b) and branch (2), both already pinned. The existing fixtures cannot see it: `split_fixed_size_walks_the_file` uses 1000/400 (last window 200) and `split_fixed_size_target_larger_than_file_is_one_task` never loops twice | `split_fixed_size_emits_the_final_one_byte_window_and_never_an_empty_vec` — pins BOTH halves of the boundary: a walk whose LAST window is exactly one byte (1000 @ 333 ⇒ `(0,333) (333,333) (666,333) (999,1)`, exact windows asserted — this half doubles as the sibling NON-VACUITY guard, since a `split` that declined everything returns one task, not four) and a one-byte parent (exactly one `(0,1)` window, never an empty Vec) | `while remaining > 0 {` → `while remaining > 1 {` → **RED**, sole killer (3166 passed / 1 failed / 1 ignored, vs 3167/0/1 green). Restored from a plain-`cp` backup, `cmp`-verified byte-identical, and the restore re-run REBUILT (`Compiling iceberg`) back to 3167/0/1 |
| F-B | S4 | the §17 doc at `arrow/reader.rs` said branch (1b)'s only remaining reachable shape "cannot be built here". FALSE, and both reviewers measured why: the shape IS reachable — `file_size_in_bytes == 0` makes `split` reach (1b) and return exactly one task `(start 0, length 0)` (code-verified: (1a)'s `start != 0 \|\| length != file_size_in_bytes` is false on `0/0/0`). What fails is the subsequent READ | sentence corrected to "reachable but not exercisable END-TO-END at the reader level", with the measured reason: `ErrorKind::Unexpected` "Failed to load Parquet metadata" (`arrow/reader.rs:1306`) with source "EOF: file size of 0 is less than footer" (`parquet-58.4.0/src/file/metadata/reader.rs:616`). Both strings grep-verified against source; the rest of the doc block was re-checked and is accurate, left intact | n/a (doc) |
| F-C | S4 | a false-green mechanism in OUR OWN verification method: the §17 Falsifier's harness restored mutations with `shutil.copy2` (= `cp -p`), putting the ORIGINAL mtime back, so cargo treated the mutant-era artifact as fresh and did not rebuild. A mutation run can then report GREEN having never compiled the restored code — and a full GATE can be served from a mutant-era binary. `cmp`/md5 passes in both cases: it proves the SOURCE was restored, not which bytes the artifact was built from | `task/lessons.md` entry in the established DO/DO-NOT form: restore with plain `cp` (or `touch` after), verify on BOTH axes (content AND a rebuild actually happened), never report a GREEN from an unconfirmed restore. Tied to the two prior anti-stale-artifact entries (2026-07-24 `cp -p` + `mv`; 2026-07-25 inherited warm `target/`) as one general rule reached from a third direction | n/a (doc) |

### 18.1 · Named residue — mutants recorded, deliberately NOT built

Recorded so the next unit can pick these up without re-deriving anything. None is a demonstrated
silent row-loss or row-duplication path, which is why each is residue rather than a defect.

| # | Exact mutant | Measured consequence | Why residue |
|---|---|---|---|
| (i) | in branches (1a), (1b), (1c) of `FileScanTask::split`: `return Ok(vec![self.clone()]);` → `return Ok(vec![self.sub_task(self.start, self.length)]);` (three mutants, applied independently) | all three **SURVIVED** at 3166/0. `sub_task` NULLs `record_count` and CLEARS `split_offsets`, so the passthrough stops being verbatim | VERBATIM passthrough fidelity is pinned on branch (1) ONLY — branch (1)'s identical mutation IS killed (`split_non_splittable_returns_self` asserts `record_count == Some(1000)`). The window is unchanged in every case, so this is metadata fidelity on a passthrough, not lost or duplicated rows. The refactor that would trip it (`self.clone()` → `self.sub_task(..)` for uniformity across the four branches) is a plausible future edit, which is why it is named |
| (ii) | `scan/mod.rs:60-62` `fn is_strictly_ascending_offsets`: `offsets.windows(2).all(\|w\| w[1] > w[0])` → `w[1] >= w[0]` | **SURVIVED**. The same relaxation on the twin — `scan/task.rs:70-72` `fn is_strictly_ascending`, `values.windows(2).all(\|pair\| pair[0] < pair[1])` → `pair[0] <= pair[1]` — is KILLED (`split_non_ascending_offsets_fall_back_to_fixed_size`) | only the `task.rs` copy is pinned. The underlying smell is the DUPLICATION itself: two byte-equivalent predicates in sibling modules, one pinned and one not, with no shared definition. Deleting the `mod.rs` copy in favour of the `task.rs` one is the real fix and is a production change — out of scope for a converging round |
| (iii) | `target.min(remaining)` → `remaining` in `split_fixed_size` (the "a `split` that declines everything" mutant, catalogued as M16 in §17/P4) | two PRE-EXISTING pins stay **GREEN** under it: `split_non_splittable_returns_self` and `split_fixed_size_target_larger_than_file_is_one_task` | both are VACUOUS under that mutant — each asserts a ONE-task outcome, which a declining `split` satisfies for the wrong reason. §17/P4 fixed exactly this shape on the rider's own pin by adding a sibling non-vacuity guard; the same one-line guard applies to each of these two. Not a defect in production code and not row loss — a test-quality debt, named with its mutant so it can be closed mechanically |

...
## Round 2 — L-001: `iceberg.schema` nested-type key order — FIXED

**Finding (Grok logic critic, verified).** `java_ordered_schema_json` ordered only the schema
wrapper; nested types still serialized through the Rust `Type` serde, which emits list
`element-required` before `element` and map `value-required` before `value`. Java
`SchemaParser.toJson` (bytecode-verified against the 1.11.0 runtime) writes:

- schema: `type`, `schema-id`, `identifier-field-ids` (only when non-empty), `fields`
- field: `id`, `name`, `required`, `type`, `doc`, `initial-default`, `write-default`
- list: `type`, `element-id`, `element`, `element-required`
- map: `type`, `key-id`, `key`, `value-id`, `value`, `value-required`

The Rust `SerdeNestedField` order already matched Java's field order; only the list/map type
wrappers diverged. Byte oracle: the Spark-written data file
`wh-metrics-loc/ns/m_default/data/…parquet` (schema `st STRUCT<a,b>`, `xs ARRAY<INT>`) — its
`iceberg.schema` kv is reproduced byte-for-byte by the pin. The map arm is verified against
Java bytecode plus the `interop/unknown_type/java.schema.json` fixture.

`identifier-field-ids`: Java writes the stored `int[]` order (whatever set order was supplied);
the fork stores a `HashSet` with no stored order, so the footer renders the set sorted — the
only deterministic choice, and equal to Java's output for the canonical `[1,2]` case. A Java
schema whose stored order is non-sorted cannot be represented in the fork's `HashSet` at all —
a representational gap above this serializer, recorded not fixed.

**Fix.** `parquet_footer.rs` gained a footer-local recursive serializer: `JavaOrderedFields` /
`JavaOrderedField` / `JavaOrderedType` walk `NestedField`/`Type` and emit every level in Java's
order, defaults via the same `Literal::try_into_json` the global serde uses. The global `Type`
serde (which writes table-metadata JSON) is deliberately untouched — Java's
`TableMetadataParser` embeds the schema through the same `SchemaParser` order, but the
rust-generated byte-pinned fixtures were produced by the old order, so a global change is a
larger, separately-scoped question (see hand-back Q2).

**Pins.** `iceberg_schema_json_matches_java_nested_key_order` (red first): cell 1 reproduces
the m_default Spark file's `iceberg.schema` byte-for-byte (list + nested struct + field +
wrapper order); cell 2 exercises map<string, struct>, a doc, `identifier-field-ids` and an
`initial-default` against the bytecode-derived Java string. Mutation: swapping
`element`/`element-required` back to Rust order fails the pin on the oracle cell — observed.
Pre-existing gap noted, not fixed: a struct-typed `initial-default` literal serializes through
`serde_json::Value` (BTreeMap key order), while Java's `SingleValueParser` writes literal
fields in schema order — same gap exists in the global serde; out of scope here.

**Gates.** fmt clean; `clippy -p iceberg --all-targets -D warnings` clean; rust-file-size 522
clean; comment-ban hits=0; footer tests 10/10; writer surface 167 green.

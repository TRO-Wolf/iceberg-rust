...
# Todo archive — June charter & 8-hour blocks (archival pass 6)

Increment plans/outcomes moved verbatim by todo-archival pass 6 (2026-07-26). Covers the
2026-06-13 superseded near-full-parity queue (its three genuinely-open items — multi-spec write
interop, geometry/geography, parked encryption — ALSO lifted to the live Carried-forward
section, the sanctioned overlap) and the 2026-06-15 → 06-19 arc: the conflict-validation +
multi-spec charter, post-charter maintenance surface, 8-hour-plan Blocks 2–10 + roadmap check,
the OverwriteFiles C1 DONE unit, and the three 2026-06-13 LANDED sections. Pass-scoped (the
pass-3 precedent). Unticked planning boxes inside DONE/LANDED sections are stale planning
churn, preserved verbatim. Not session-start reading.

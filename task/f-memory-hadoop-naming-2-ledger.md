...
| C-10 | (r5) Registered `v2000000000.metadata.json` on a storage whose `list` fails `FeatureUnsupported`: drop returns `Ok` after exactly 2 deletes (the current file and the hint) and 1 list call; the pointer and a hand-placed `version-hint.text` are each absent by name (r9, critic V-010; the count alone could not tell a hint delete from a second current-file delete). The counting storage fails any delete past 64 per catalog, so a reintroduced walk fails instead of hanging | `hadoop_drop_of_huge_registered_version_without_listing_is_bounded` |
...
| C-26 | (r8, critic V-009) Hadoop mode on a storage whose `list` waits on a gate, table at `v1` and, separately, at `v2`: a `drop_table` is held at its listing (the current file already deleted) while a `create_table` of the same name is started through a test-only wrapper that signals when it enters `create_table` (r9, critic V-012: a rendezvous, no yield count; on the current-thread runtime the creator runs to its first await, the contested pointer lock, before the test resumes); after the gate opens, the drop and the create both return `Ok`, the new pointer is `v1`, its file exists, the hint reads `1`, and the create had not finished while the drop was at its listing. Red before the fix (`v1` case): `load: DataInvalid => File not found: memory:///warehouse/ns/t/metadata/v1.metadata.json` | `race_tests::hadoop_create_racing_a_drop_keeps_its_v1` (`hadoop_drop_race_tests.rs`) |
| C-27 | (r9, critic V-013) Memory storage keeps `/warehouse/ns/t/metadata/...` and `file:/warehouse/ns/t/metadata/...` as distinct keys, while the directory match maps both to one key. Two Hadoop tables registered at those two locations, each with `v1`, `v2` and a hint; dropping either one removes its own three files by name and leaves the other's three files with their bytes and its table loadable at `v2`, in both directions | `path_tests::hadoop_drop_keeps_a_scheme_variant_sibling_directory` (`hadoop_drop_path_tests.rs`) |
...
Round r9 (critic r6) mutation checks, each reverted with `git checkout`:

| Mutation | Red |
|---|---|
| (r9-h1) the no-list arm deletes the current file a second time and skips the hint (still 2 deletes) | C-10 (`version-hint.text`, the by-name assertion; the count assertion stays green), C-8 |
| (r9-r1) the Hadoop arm of `drop_table` releases the pointer lock before cleanup, run 5 times | C-26, 5 of 5 (`load: DataInvalid => File not found: memory:///warehouse/ns/t/metadata/v1.metadata.json`) |
| (r9-m) the drop lists `storage_path(metadata_dir)` instead of the pointer's own directory | C-27, C-11, C-6 |
...
- D-2 (revised in r2): for a `vN` pointer the helper lists the pointer's metadata directory once
  ...
  parses as Hadoop convention with `1 <= K <= N`, and whose parent key equals the pointer's
  directory key. This is equality of keys returned by listing the pointer's own metadata directory,
  not directory identity (r9, critic V-013): the normalization below maps `file:/warehouse` and
  `/warehouse` to one key although `MemoryStorage` keeps them distinct, and no over-delete follows
  because the listing is confined to the pointer's own prefix (C-27, mutation (r9-m)).
...
- (r9, critic V-011) A Hadoop `drop_table` future cancelled at the `cache_invalidate` await, which
  runs after the pointer is removed and before any file delete, leaves the current file, the earlier
  chain and the hint, with no pointer for a retry (same end state as cases 1 to 3). Not pinned:
  with no cache injected the await completes at once, and an injected cache offers no hook to hold
  it pending.

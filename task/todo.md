...
off fork `main`. Consumer: RePark rating row C-3 / V3-02 (registry V3-COV-3).

- [x] RED: `insert_row_id_order.rs` — five real-SQL cells over a four-partition `MemTable` on a
      multi-thread runtime, 20 fresh v3 tables each; the SELECT / overwrite / eight-partition
      cells red at run 0 pre-fix. `1920c8833`
- [x] FIX: `IcebergWriteExec` emits `write_partition_index`; `IcebergCommitExec` stable-sorts
      the collected list by `ascending_partition_order` then that index — writer order within
      one input partition, then input partition index. `01b1a2f4`
...
- [ ] RePark: delete `legacy_deletes.rs` own walk; consume `DvContainerClose::legacy_deletes` + `load_legacy_positions_by_path` once per delete file, then `load_legacy_positions` for a path lookup. F-23: `data_sequence_numbers` is empty when `pending_legacy` is empty and `known_partitions` covers every touched path — keep supplying that map (the skip sink). `Option<&ManifestList>` is for RePark.
...
- [x] C-003 pins: base cell, two file-scoped parquet on one data file (DV rc 3), UPDATE, untouched-file control, partition-scoped merge-and-keep (DV rc 2 + parquet 1), sequence-number not-apply (warehouse parquet); 6 tests; Java interop `run-interop-f21-legacy-delete-merge.sh` (2 fixtures, 3 sabotages red). Mutations recorded in the ledger as N red of 6.
...
- [x] Round 5: re-measure mutations of 6; regenerate Files changed from `2e9b5b591..HEAD`; C-003 6 tests / 2 fixtures / 3 sabotages; `verifyPartition` format-version 3 guard; drop measure-test elapsed asserts
...
- [x] C-003 lazy walk, PINNED by a counting `Storage`: without a partition map each data manifest is read exactly once. F-22 reversed the supplied-map ZERO-read so sequence numbers always fill; F-23 restored the skip when `pending_legacy` is empty and `known_partitions` covers every touched path. Round 2 added `with_previous_deletes` for the touched blob, the untargeted-manifest early-out, the caller-supplied partition map, and the borrowed streaming manifest reads.
...
- [x] `next-row-id` is Java `+= existing+added` on unassigned DATA (layout-dependent). F-rp3-c7 sequential-COW mismatch was a two-file Spark-seed artefact.
...
- [x] Delta critic: Spark 1.11 seven-sequence table; mixed probe; `unassigned_row_count` on `ManifestFile`; C-007 restated as Spark fidelity.

...
- **DO state the scan/commit split on `with_commit_branch`.** *Why:* the setter is a commit target, not `asBranch`. Callers otherwise assume DELETE/UPDATE read the named branch. _superseded 2026-09-01: F-6c makes scans follow the named branch; the split sentence is gone. OCC now arms `validate_from_snapshot` with the scanned snapshot instead of skipping._

### 2026-09-01 — DataFusion `with_commit_branch` follows the named branch for reads as well as commits

- **DO resolve `snapshot_for_ref` at every DataFusion scan when `commit_branch` is set, including CoW/MoR live-file walks.** *Why:* F-6b committed onto the named ref while scans still read `main`. A diverged DELETE then deleted the wrong rows. A pin that deletes a row present on both heads (`id = 1`) stays green under a main-read. Use a branch-only predicate (`id = 10`). Pins: `copy_on_write_update_on_diverged_branch_follows_branch_only_row`, `merge_on_read_update_on_diverged_branch_follows_branch_only_row`, `merge_on_read_delete_on_diverged_branch_follows_branch_only_row`, `copy_on_write_delete_on_diverged_branch_follows_branch_only_row`.
- **DO error on a missing ref at the read leg (`snapshot ref '{name}' not found`), and DO keep INSERT-without-a-target-scan creating the branch.** *Why:* Java `useRef` is loud; `to_branch` creates a missing ref at commit. A DML that scans (DELETE/UPDATE) never reaches commit. Pin: `delete_missing_branch_errors_on_the_read_leg`.
- **DO arm `validate_from_snapshot` with the scanned snapshot, not with `main` and not by skipping.** *Why:* the F-6b skip existed only to avoid passing `current_snapshot_id()` (main). Once the scan is the named-ref head, that id is the correct OCC start. Pin: `insert_overwrite_on_diverged_branch_does_not_treat_branch_files_as_concurrent`.

### 2026-09-03 — A lookup helper must not re-read, and a falsified F-18 pin must be rewritten not left named

- **DO make `load_legacy_positions` a lookup on a loaded index (`&HashMap` → `&[u64]`), never a second parquet read.** *Why:* F-22 round 2 left a `///` that said "from a loaded index" while the body called `load_legacy_positions_by_path`. A per-path caller got the 63 s behaviour and 0 red of 13. Pin parquet `opens` so one-read-per-delete-file is mutation-red.
- **DO rewrite live text when a later unit reverses a pin (GAP_MATRIX clause, `task/todo.md`, test name).** *Why:* F-22's unconditional data-manifest walk for sequence numbers made F-18's "supplied partition map reads ZERO data manifests" false while the test name still said `reads_no_data_manifest`. A full-map caller that wants no sequence numbers has no opt-out; say so in the ledger.

### 2026-09-13 — `#[typetag::serde]` traits need serde impls on test doubles; shrinking a legacy file means lowering its ceiling

- **DO give test-only `Storage`/`StorageFactory` wrappers failing `Serialize`/`Deserialize` impls plus `#[typetag::serde(name = ...)]`.** *Why:* both traits are `#[typetag::serde]`-registered, so a plain `impl` fails with missing `typetag_name`/`typetag_deserialize` and a missing `erased_serde::Serialize` bound; wrappers holding `Arc<dyn Storage>`/logs cannot derive serde. Erroring manual impls satisfy the bound and are never invoked. Used by the counting `FileRead` harness in `rewrite_size_shared/counting.rs` and `rewrite_data_files_fuse_tests.rs`.
- **DO lower a `LEGACY_CEILINGS` entry in `scripts/check_rust_file_size.py` when a listed file shrinks; do NOT treat "under ceiling" as done.** *Why:* the gate fails both ways — grow past the ceiling or shrink below it without updating the row. Extracting code out (e.g. `arrow/open_parquet.rs` from `reader.rs`) is the sanctioned move; the ceiling follows the file down.
- **DO pin footer fetches as tail-reaching reads (`range.end == file_size`), not raw read counts.** *Why:* parquet-rs splits metadata loading into an 8-byte tail read plus a non-overlapping exact footer read, and index loads are a third range; counting all reads conflates shapes. The C-009 pin counts only reads that reach the file tail and asserts no two ranges overlap — robust across reader internals.

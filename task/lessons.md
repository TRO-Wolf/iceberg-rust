...
- **DO resolve `snapshot_for_ref` at every DataFusion scan when `commit_branch` is set, including CoW/MoR live-file walks.** *Why:* F-6b committed onto the named ref while scans still read `main`. A diverged DELETE then deleted the wrong rows. Pins: `scan_with_commit_branch_returns_branch_rows_not_main`, `delete_with_commit_branch_follows_branch_rows_and_leaves_main_untouched`.

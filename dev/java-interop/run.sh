...
# under the License.
#
# UpdateSchema + UpdatePartitionSpec bidirectional interop harness runner.
#
# This is a TEST-ONLY ORACLE (a dev tool, like dev/spark/) — it is NOT part of the shipped Rust library.
# It proves byte-/field-id-level UpdateSchema AND UpdatePartitionSpec compatibility with the Java
# `iceberg-core` reference in BOTH directions and regenerates the committed JSON fixtures. One Java
# `generate`/`verify` pass covers both capabilities (the Java oracle drives both scenario registries):
#
#   1. mvn ... -Dexec.args=generate
#        -> The Java oracle (re)writes base.metadata.json + java_evolved.metadata.json for EACH scenario
#           of BOTH capabilities (update_schema/ + update_partition_spec/).
...
#        -> The Java oracle reads each rust_evolved.metadata.json (both capabilities) and asserts Java
#           parses it and its current schema / default partition spec matches Java's own evolution
#           (Direction 2 verifier). Exits non-zero on any FAIL.
#
...
echo "==> [1/4] Java oracle: generate base + java_evolved fixtures (schema + partition)"
...
echo "==> [3/4] Java oracle: verify (Direction 2 — Java reads Rust output; schema + partition)"
"${MVN}" -f "${SCRIPT_DIR}" -q exec:java -Dexec.args=verify

echo "==> [4/4] Done — both directions passed (UpdateSchema + UpdatePartitionSpec)."

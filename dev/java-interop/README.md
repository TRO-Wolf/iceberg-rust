...
- **Dynamic discovery** — the suite set is globbed at run time, so a new `run-interop-*.sh`
  ...
  authoring, 2026-07-10) in the same change that adds a suite** — discovery below the floor
  fails the run, so an accidentally-emptied glob cannot green.
...
- Runner provisioning: all 48 suites default to `/opt/maven/bin/mvn` and
  `JAVA_HOME=/usr/lib/jvm/java-11-openjdk-amd64` (47 hardcode them outright; only
  ...
  once, because 47 of the 48 suites invoke `mvn -o` (offline).

...
#     spec::manifest::entry::first_row_id_tests, not by this leg.
...
echo "==> [3/5] (D1) Rust: build the same lineage view of the JAVA table and diff"
(
  cd "${REPO_ROOT}"
...
if echo "${VERIFY_OUT}" | grep -q '^FAIL ' \
  || ! echo "${VERIFY_OUT}" | grep -q 'verify-interop-row-lineage: 0 failures'; then

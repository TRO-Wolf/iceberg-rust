...
| `run-interop-write-data.sh` | S1+W1 data-level write-actions interop: FOUR FIXTURES, 12 steps, REAL parquet, both directions + 2nd-pass repeat + sabotage battery. Fixture A (merge_append, S1): V2 partitioned table (`identity(category)`), fast_append A+B → set min-count=2 → merge_append G; live rows `{10,20,30,40,60}`. Fixture B (rewrite_files, S1, seq-preservation): unpartitioned 2-field table, fast_append A → equality-delete(ids 20+40, seq 2) → rewrite {A}→{A'} with `data_sequence_number(1)` — proves A'.data_seq=1 < eq_del.seq=2; live rows `{10,30,50}`. Fixture C (overwrite_files, W1): V2 partitioned table, fast_append A+B → overwrite_files DELETE B ADD B'(cat=b,id=41,d'); live rows `{10,20,30,41}` (id=40 absent). Fixture D (delete_files, W1): V2 partitioned table, fast_append A+B+C_file → delete_files {B}; live rows `{10,20,30,50}` (id=40 absent). Per fixture: Java writes+emits ground truth (step 2), Rust writes+Java verifies (steps 3-7), Rust reads Java table (step 8). Steps 9-11: 2nd pass (determinism). Step 12: sabotage battery (metadata corruption → verify must fail). Partition column pinned in ALL fixtures (S3 partition-projection lesson: `id_to_category_sorted` compare catches wrong-partition writes invisible to the `{id,data}` row compare). Separate from `run-interop-write-actions.sh` (metadata-level only). Consumer: `tests/interop_write_data.rs`. |

...
| DELETE | MoR (`RowDelta`) | snapshot | *base only* for V2 — **correction (2026-07-09): Java enables `validate_deleted_files()` for UPDATE/MERGE ONLY** (`SparkPositionDeltaWrite.java` L251-254). V2 MoR DELETE still skips `Operation::Delete` in files-exist. **V3 DataFusion DELETE (F-17, named Java divergence; narrowed by F-19):** arms `validate_deleted_files()` over replacement DV references only (`DvContainerClose::referenced_data_files`). Untouched shared-Puffin siblings are not in that set, so a concurrent compaction or `DeleteFiles` of a sibling COMMITS (Java files-exist covers only referenced files). Unrelated files stay outside that set. | `SparkPositionDeltaWrite.commit` L240-249 · V2: interop `interop_rowdelta_conflict.rs` files-exist axis + unit `row_delta.rs::test_row_delta_files_exist_skip_deletes_default_excludes_delete_op_snapshot` (half A). V3: `shared_puffin_dv` F-19a pins |

...
| DELETE | MoR (`RowDelta`) | snapshot | *base only* — **correction (2026-07-09): the previous draft prescribed `validate_deleted_files()` here; Java enables it for UPDATE/MERGE ONLY** (`command == UPDATE \|\| command == MERGE`, `SparkPositionDeltaWrite.java` L251-254). A MoR DELETE tolerates a concurrent DELETE-op removal of a referenced file being outside the default `{OVERWRITE, REPLACE}` op set (a concurrent compaction/REPLACE removal is NOT tolerated — `REPLACE` is in both op sets) | `SparkPositionDeltaWrite.commit` L240-249 · interop `interop_rowdelta_conflict.rs` files-exist axis (`files_exist_{reject,accept}`) + unit `row_delta.rs::test_row_delta_files_exist_skip_deletes_default_excludes_delete_op_snapshot` (half A = the corrected no-`validate_deleted_files` behavior); commit-leg observable in `engine_contract_isolation_recipes.rs` |

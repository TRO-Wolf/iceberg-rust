...
| DELETE | MoR (`RowDelta`) | snapshot | *base only* for V2 — **correction (2026-07-09): Java enables `validate_deleted_files()` for UPDATE/MERGE ONLY** (`SparkPositionDeltaWrite.java` L251-254). V2 MoR DELETE still skips `Operation::Delete` in files-exist. **V3 DataFusion DELETE (F-17, named Java divergence):** arms `validate_deleted_files()` over every replacement DV reference, including copied shared-Puffin siblings, so a concurrent `DeleteFiles` of a sibling is rejected. Unrelated files stay outside that set. | `SparkPositionDeltaWrite.commit` L240-249 · V2: interop `interop_rowdelta_conflict.rs` files-exist axis + unit `row_delta.rs::test_row_delta_files_exist_skip_deletes_default_excludes_delete_op_snapshot` (half A). V3: `shared_puffin_dv.rs` + `delete_vector_container.rs` |

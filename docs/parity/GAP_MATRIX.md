...
1. **Write engine** — everything beyond fast-append (`RowDelta`, `RewriteFiles`, `RewriteManifests`,
   merge append). **Started 2026-06-07: `DeleteFiles` 🟡 + `OverwriteFiles` 🟡 + `ReplacePartitions` 🟡.**
   ...
   unpartitioned table is a full replace; `replace-partitions=true` summary marker). All three
   `MemoryCatalog`-tested (post-commit SCAN live set + cross-partition isolation + provenance). Deferred:
   `overwriteByRowFilter` / delete-by-row-filter / static `replaceByRowFilter` (metrics evaluators),
   concurrent-commit conflict validation (serializable isolation), and data-level Java interop.

...
2. **Schema/partition evolution + snapshot management** — `UpdateSchema`, `UpdatePartitionSpec`, and
   `ManageSnapshots` are all 🟡 (landed with unit tests; **Java interop round-trip is the only thing left
   before ✅** for each). `UpdateSchema` column initial/write **defaults** are now plumbed; `ManageSnapshots`
   now has `rollbackToTime` + non-positive-retention rejection (`cherrypick` is Phase-2-gated — it extends

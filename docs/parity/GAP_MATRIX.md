...
1. **Write engine** — everything beyond fast-append (`ReplacePartitions`, `RowDelta`, `RewriteFiles`,
   `RewriteManifests`, merge append). **Started 2026-06-07: `DeleteFiles` 🟡 + `OverwriteFiles` 🟡.**
   ...
   current_data_manifests}` (factored out of `DeleteFiles`). Both `MemoryCatalog`-tested. Deferred:
   `overwriteByRowFilter` / delete-by-row-filter (metrics evaluators), concurrent-commit conflict
   validation (serializable isolation), and data-level Java interop.

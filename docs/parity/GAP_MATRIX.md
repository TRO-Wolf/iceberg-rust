...
2. **Schema/partition evolution + snapshot management** — `UpdateSchema`, `UpdatePartitionSpec`, **and the
   ...
   **ref-operation surface** of the **"Snapshot management"** row are now ✅; **`cherrypick` remains
   ...
   table class Java emits. (The sibling spec-mandated default-to-0 read fields — `last-sequence-number` on
   table metadata; manifest-list `sequence-number`/`min-sequence-number`; manifest/manifest-list `content`
   — were NOT verified or fixed in that change; tracked as a future reader-robustness pass.)
